...
0. ✅ **RESUELTO/DEPLOYADO (2026-06-24):** el `deploy.sh` de la sesión de Reseñas empaquetó TODO el working tree → el color **AZUL MARINO**, `login.html` (aurora) y el backend de **Meta Ads/Google stats** quedaron **en producción** y la app levanta OK (health check pasó). **Pendiente de LIMPIEZA GIT (importante):** `lib/google-stats.js` y `lib/meta-ads.js` siguen **untracked** aunque `index.js` (commiteado) los `require` → un checkout/clone limpio rompe la app (ver BUGS). Commitear esos 2 lib + los modificados sin commitear (`lib/generador.js`, `lib/drive.js`, `lib/referencia.js`, `public/admin/login.html`, `supabase-web-analytics.sql`) en un commit, **confirmando antes que el backend de Meta Ads esté validado en prod** (no lo verifiqué end-to-end esta sesión).
