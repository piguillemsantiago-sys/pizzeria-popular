...
tar czf - --exclude=node_modules --exclude=.git --exclude=.env --exclude=google-ratings.json --exclude=ig-token.json --exclude=google-oauth-token.json --exclude=gbp-locations.json --exclude=gbp-auto-state.json --exclude=referencia.html -C "$DIR" . \
