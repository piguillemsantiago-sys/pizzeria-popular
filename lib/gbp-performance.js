...
// Búsquedas con las que aparece la ficha (granularidad mensual, últimos 3 meses cerrados).
async function fetchKeywords(token, locId) {
  const hoy = new Date();
  const finM = new Date(hoy.getFullYear(), hoy.getMonth() - 1, 1);  // mes pasado
  const iniM = new Date(hoy.getFullYear(), hoy.getMonth() - 3, 1);
...
async function rendimiento({ local_id, desde, hasta } = {}) {
  ...
  for (const slug of slugs) {
    ...
    (await fetchKeywords(token, locId)).forEach((k) => {
