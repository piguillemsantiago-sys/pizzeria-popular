...
function ranking(doc, empleados) {
  ...
  const anchoBarra = CONTENT_W - 190;
  empleados.forEach((e, i) => {
    ...
    doc.fillColor(COLOR.texto).font(F.bold).fontSize(10.5)
      .text(num(e.menciones), x0 + anchoBarra + 10, y + 5, { width: 48, align: 'right' });
    doc.y = y + 26;
  });
}
  ...
  e.frases.forEach((f) => {
    ...
    doc.fillColor(COLOR.suave).font(F.bold).fontSize(8)
      .text(limpiar((f.autor || 'Cliente') + '  ·  ' + fechaLarga(f.fecha) +
        (informe.local_id ? '' : '  ·  ' + f.local)), M + 14, y + 1, { width: CONTENT_W - 90 });
...
function generar(informe, opts = {}) {
  return new Promise((resolve, reject) => {
    ...
    doc.fillColor(COLOR.texto).font(F.bold).fontSize(13).text('Ranking de menciones', M, doc.y);
    doc.y += 14;
    ranking(doc, informe.empleados);

    doc.moveDown(0.8);
    saltoSiHaceFalta(doc, 46);
