...
// Locales cerrados: sus reseñas se siguen sincronizando y cuentan en los
// totales, pero no se listan como local propio en el panel (cerrado 1/9/2026).
const CERRADOS = new Set(['santa-clara']);
...
async function salud(query = {}) {
  ...
  const porLocal = {};
  for (const slug of Object.keys(LOCALES)) if (!CERRADOS.has(slug)) porLocal[slug] = [];
...
{"positivo":[{"tema":"frase corta","veces":N}],"negativo":[{"tema":"frase corta","veces":N}],"platos":[{"plato":"nombre del plato","variantes":["plato","variante1"]}],"idiomas":{"es":N,"en":N,"fr":N,"otros":N}}
Reglas:
...
- NO cuentes menciones de platos: de eso se encarga el sistema contando las variantes que le pases. Lo que importa es que "variantes" esté completo.
...
- Frases cortas, español rioplatense, sin humo.`;
  ...
  const result = {
    ...
    // Los empleados mencionados salieron de acá: la lista de variantes la
    // armaba la IA en cada corrida, así que el mismo rango daba números
    // distintos (Cata 111/115/121) y colaba falsos positivos ("August" como
    // variante de Agustina). El conteo por persona ahora vive en
    // lib/menciones.js, que descubre con IA pero cuenta con reglas fijas.
