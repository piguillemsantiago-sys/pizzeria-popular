...
// 100% solo si de verdad no queda ninguna sin responder (redondear 99,93 a 100
// hacía que el panel cantara "100%" con pendientes en la lista).
function tasa(respondidas, total) {
  if (!total) return null;
  if (respondidas >= total) return 100;
  return Math.min(99, Math.round((respondidas / total) * 100));
}
...
async function metricas(query = {}) {
  ...
  if (query.hasta) {
    ...
    if (!isNaN(h.getTime())) { h.setHours(23, 59, 59, 999); hasta = h.toISOString(); }
  }

  // PostgREST corta en 1000 filas por request (el .limit(10000) no sirve):
  // hay que paginar o las tarjetas se quedan clavadas en 1000.
  const data = [];
  const STEP = 1000;
  for (let from = 0; ; from += STEP) {
    let q = supabaseAdmin.from(TABLE)
      .select('estrellas, estado, fecha_resena, fecha_respuesta')
      .eq('origen', 'google')   // mismo criterio que salud(): sin las de prueba de Fase 1
      .gte('fecha_resena', desde.toISOString())
      .order('fecha_resena', { ascending: false })
      .order('id', { ascending: false });
    if (hasta) q = q.lte('fecha_resena', hasta);
    if (local_id && validateLocal(local_id)) q = q.eq('local_id', local_id);
    const { data: page, error } = await q.range(from, from + STEP - 1);
    if (error) throw err(500, error.message);
    data.push(...(page || []));
  ...
  return {
    ...
    tasa_respuesta: total > 0 ? tasa(respondidas, total) : null,
...
async function salud(query = {}) {
  ...
  for (let from = 0; ; from += STEP) {
    ...
    if (desde) q = q.gte('fecha_resena', desde);
    if (hasta) q = q.lte('fecha_resena', hasta);
    // Sin ORDER BY el paginado por range() puede repetir o saltear filas.
    q = q.order('fecha_resena', { ascending: false }).order('id', { ascending: false });
  ...
  for (const [slug, list] of Object.entries(porLocal)) {
    ...
    out[slug] = {
      ...
      faltan_5: faltan,
      target,
      tasa_respuesta: tasa(respondidas, n),
      evolucion,
    };
  }
...
async function vozCliente() {
  ...
  ((ratings && ratings.locals) || []).forEach((l) => {
    ...
    total += revs.length;
    bloques.push('### ' + l.name + '\n' + revs.map((r) => '- ' + r.rating + '★: ' + recortar(r.text, 320)).join('\n'));
  });
...
// Recorta el texto sin partir emojis al medio: cortar en la mitad de un par
// suplente deja un carácter huérfano y la API de Anthropic rechaza el body
// entero ("no low surrogate in string") → se caía todo el análisis.
...
// Minúsculas y sin tildes: "Inés" y "ines" tienen que contar igual.
function normalizar(s) {
  return String(s || '').toLowerCase().normalize('NFD').replace(/[̀-ͯ]/g, '').trim();
}

// Cuántas reseñas del conjunto nombran algo (una reseña cuenta UNA vez aunque
// lo repita). Los nombres de 3 letras (Sol, Ana) piden mayúscula en el texto
// original, si no "sol" se lleva puesto "nos sentamos al sol".
function contarEnTextos(items, patrones) {
  const esc = (p) => p.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
  const largos = patrones.filter((p) => p.length > 3).map(esc);
  const cortos = patrones.filter((p) => p.length <= 3).map(esc);
  const reLargos = largos.length ? new RegExp('(^|[^a-z0-9])(' + largos.join('|') + ')([^a-z0-9]|$)') : null;
  const reCortos = cortos.length
    ? new RegExp('(^|[^\\wÁÉÍÓÚÑ])(' + cortos.map((p) => p.charAt(0).toUpperCase() + p.slice(1)).join('|') + ')([^\\wáéíóúñ]|$)')
    : null;
  return items.filter((x) => {
    const t = x.completo || x.texto || '';
    if (reLargos && reLargos.test(normalizar(t))) return true;
    return !!(reCortos && reCortos.test(String(t)));
  }).length;
}
...
async function insights(query = {}) {
  ...
  // TODAS las reseñas con texto del rango (paginando el cap de 1000 de
  // PostgREST). El universo completo se usa para CONTAR; a la IA se le manda
  // una muestra acotada solo para que descubra nombres, platos y temas.
  ...
  let textos = data.map((r) => ({
    ...
    texto: recortar(r.texto_original, 300),
    // El texto entero solo para contar: la IA lee 300 caracteres, pero un nombre
    // nombrado al final de una reseña larga igual tiene que sumar.
    completo: String(r.texto_original || '').replace(/\s+/g, ' '),
  }));
  ...
  if (textos.length < 30 && !desde && !hasta) {
    ...
    ((ratings && ratings.locals) || []).forEach((l) => {
      ...
      (l.topReviews || []).forEach((r) => {
        ...
        if (t) textos.push({ local: l.name, estrellas: (r && r.rating) || 5, texto: recortar(t, 300) });
      });
    });
  }
  ...
  // Muestra para la IA: si el rango tiene más de MUESTRA_MAX, se toma repartida
  // a lo largo del período (antes eran solo las más recientes, así que un rango
  // largo se analizaba con los últimos meses nada más).
  const universo = textos;
...
{"positivo":[{"tema":"frase corta","veces":N}],"negativo":[{"tema":"frase corta","veces":N}],"empleados":[{"nombre":"Nombre","variantes":["Nombre","variante1"],"nota":"por qué lo mencionan"}],"platos":[{"plato":"nombre del plato","variantes":["plato","variante1"]}],"idiomas":{"es":N,"en":N,"fr":N,"otros":N}}
Reglas:
...
- empleados: SOLO nombres propios de personas mencionadas en el texto (no "el camarero"). Hasta 8.
- empleados, UNIFICAR variantes: el mismo nombre escrito distinto es LA MISMA persona — con/sin tilde (Ines=Inés), errores de tipeo o letras cambiadas (Sergi/Serghio=Sergio, Inez/Ynes=Inés, Kata/Katta=Cata, c↔k, s↔z, i↔y), apodo/diminutivo vs nombre completo (Cata=Catalina, Agus=Agustina, Nacho=Ignacio). Usá como etiqueta la forma más frecuente y listá en "variantes" TODAS las formas que viste escritas, incluida la etiqueta.
- NO cuentes menciones de empleados ni de platos: de eso se encarga el sistema contando las variantes que le pases. Lo que importa es que "variantes" esté completo.
- platos: platos/productos concretos nombrados (fugazzeta, milanesa, tiramisú...). Hasta 8, con sus variantes de escritura (singular/plural incluidos).
- idiomas: contá en qué idioma está escrita cada reseña.
- Frases cortas, español rioplatense, sin humo.`;

  const payload = JSON.stringify({
    model: MODELO,
    max_tokens: 4000,   // las listas de variantes engordan el JSON; con 2000 se cortaba
    system,
    messages: [{
      role: 'user',
  ...
  // Los conteos de empleados y platos NO salen de la IA (los estimaba y erraba
  // hasta un 50%, y cambiaban en cada corrida): se cuentan acá, sobre TODAS las
  // reseñas del rango, buscando las variantes que la IA reconoció.
  const contar = (etiqueta, variantes) => {
    const pats = [...new Set([etiqueta, ...(Array.isArray(variantes) ? variantes : [])]
      .map((v) => normalizar(v)).filter((v) => v.length >= 3))];
    if (!pats.length) return 0;
    return contarEnTextos(universo, pats);
  };
  ...
  const result = {
    ...
    empleados: lim(parsed.empleados, 8)
      // variantes viaja al cliente: el informe de menciones (lib/menciones.js)
      // las reusa como semilla para su propio conteo.
      .map((e) => ({ nombre: e.nombre, nota: e.nota, variantes: e.variantes || [], menciones: contar(e.nombre, e.variantes) }))
      .filter((e) => e.nombre && e.menciones > 0)
      .sort((a, b) => b.menciones - a.menciones),
    platos: lim(parsed.platos, 8)
      .map((p) => ({ plato: p.plato, menciones: contar(p.plato, p.variantes) }))
      .filter((p) => p.plato && p.menciones > 0)
      .sort((a, b) => b.menciones - a.menciones),
    idiomas: parsed.idiomas || {},
    muestra: universo.length,      // reseñas con texto del rango (sobre las que se cuenta)
    analizadas: textos.length,     // las que leyó la IA para descubrir nombres y temas
    generado: hoy,
  };
