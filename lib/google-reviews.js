...
async function salud() {
  if (_saludCache.data && Date.now() < _saludCache.hasta) return _saludCache.data;
  ...
  const data = { locales: out, generado: new Date().toISOString() };
  _saludCache = { hasta: Date.now() + 10 * 60 * 1000, data };
  return data;
}
...
async function insights(query = {}) {
  ...
  // Reseñas con texto de la BD (las más recientes primero, tope 400).
  let q = supabaseAdmin.from(TABLE)
