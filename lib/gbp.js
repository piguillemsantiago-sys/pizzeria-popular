...
const CERRADOS = new Set(['santa-clara']); // cerró 1/9/2026
...
function toRow(localId, rev) {
  ...
  return {
    ...
    // Local cerrado: se guarda y suma, pero nadie la responde → nace descartada.
    estado: reply ? 'respondida' : (CERRADOS.has(localId) ? 'descartada' : 'pendiente'),
...
// ---- Nota y nº de reseñas del local → tabla restaurants ----
// La respuesta de reviews.list ya trae averageRating y totalReviewCount (los
// mismos números que la ficha pública), así que esto no cuesta ninguna llamada
// extra. La carta digital (Railway) lee esas dos columnas para su portada.
async function guardarRating(localId, averageRating, totalReviewCount) {
  const rating = Number(averageRating);
  const reviews = Number(totalReviewCount);
  // Si Google no los mandó, no pisar lo que ya había con null/0.
  if (!Number.isFinite(rating) || !Number.isFinite(reviews) || reviews <= 0) return;
  const { error } = await supabaseAdmin.from('restaurants')
    .update({
      google_rating: Math.round(rating * 10) / 10,
      google_reviews_count: reviews,
    })
    .eq('slug', localId);
  if (error) console.error('[GBP] Rating ' + localId + ':', error.message);
}
...
async function syncLocal(localId, v4Name, { full = false } = {}) {
  ...
  do {
    ...
    // La 1ª página ya trae los totales del local; se guardan aunque no haya
    // reseñas nuevas (si no, el corte incremental de abajo saltearía esto).
    if (primeraPagina) {
      primeraPagina = false;
      await guardarRating(localId, r.averageRating, r.totalReviewCount);
    }
...
// ---- Estado de apertura de una ficha (Business Information v1) ----
// v1 usa locations/{id} (sin la cuenta); lo sacamos del v4 guardado.
function v1Name(slug) {
  const m = loadLocations();
  const loc = m.locales && m.locales[slug];
  if (!loc) throw new Error('Local sin mapear: ' + slug);
  return 'locations/' + loc.v4.split('/locations/')[1];
}
async function openInfo(slug) {
  const r = await gapi(BI + v1Name(slug) + '?readMask=openInfo,title');
  return { title: r.title, openInfo: r.openInfo || null };
}
// Marca la ficha como cerrada permanentemente (irreversible desde acá).
async function cerrarPermanente(slug) {
  return gapi(BI + v1Name(slug) + '?updateMask=openInfo.status', {
    method: 'PATCH',
    body: JSON.stringify({ openInfo: { status: 'CLOSED_PERMANENTLY' } }),
  });
}
module.exports.openInfo = openInfo;
module.exports.cerrarPermanente = cerrarPermanente;
