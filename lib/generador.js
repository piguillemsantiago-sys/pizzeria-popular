...
function svgTexto(placa, W, H, numero, total, zona) {
  ...
  // La zona (de la visión) manda sobre la posición; si no hay, usa el estilo.
  const vert = zona && zona.vert ? zona.vert : (est.pos === 'top' ? 'arriba' : est.pos === 'center' ? 'centro' : 'abajo');
  const align = zona && zona.horiz ? zona.horiz : est.align;
  ...
  // Título blanco (Montserrat Bold) con sombra.
  for (const line of tLines) {
    y += tLH;
    const attrs = 'font-family="Montserrat" font-weight="bold" font-size="' + tSize + '" letter-spacing="1"' + anchor;
    s += txtSombra(tx, y, attrs, escXml(line), '#fff');
  }

  // Acento dorado en cursiva (Great Vibes) — el remate emocional, con sombra.
  if (aLines.length) {
    y += gapTA;
    for (const line of aLines) {
      y += aLH;
      const attrs = 'font-family="Great Vibes" font-size="' + aSize + '"' + anchor;
...
Devolvé SOLO un JSON: { "vert": "arriba"|"abajo"|"centro", "horiz": "izquierda"|"derecha"|"centro" }
