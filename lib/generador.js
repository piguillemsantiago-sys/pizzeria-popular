...
async function overlayTituloCaptura(imgBuf, titulo, diseno) {
  ...
  // Apilado del bloque: arriba (diseño 'arriba') o tercio inferior (el resto).
  ...
  const scrim = '<svg xmlns="http://www.w3.org/2000/svg" width="' + W + '" height="' + H + '">' +
    ...
    (esArriba
      ? '<rect x="0" y="0" width="' + W + '" height="' + (210 + altoBloque) + '" fill="url(#t)"/>' +
        '<rect x="0" y="' + (H - 320) + '" width="' + W + '" height="320" fill="url(#b)"/>'
      : '<rect x="0" y="' + (1690 - altoBloque) + '" width="' + W + '" height="' + (H - (1690 - altoBloque)) + '" fill="url(#b)"/>') +
    '</svg>';

  const conTexto = await sharp(imgBuf)
    .composite([{ input: Buffer.from(scrim), top: 0, left: 0 }, ...capas])
    .png().toBuffer();
  // Wordmark real centrado abajo, como en las placas de referencia.
  const logo = await prepararLogo('wordmark-blanco', W, H, 'abajo');
