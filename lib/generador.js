...
// Timeout corto y explícito (13 jul): el default del SDK son 10 minutos — una
// llamada colgada dejaba al panel en "Ajustando…" hasta que nginx cortaba en
// 504. Con 2 min por llamada + 1 retry del SDK, lo peor son ~4 min y el error
// llega al usuario como mensaje accionable, no como espera eterna.
...
REGLA DE ORO (por encima de todas): si la instrucción DICTA textos — entre comillas,
o con "que diga...", "título: ...", "el botón dice..." — esos textos van EXACTOS,
letra por letra, en el campo que corresponda. NO los embellezcas, NO los completes,
NO los reescribas "mejor". Tu creatividad es SOLO para los campos que el usuario NO
dictó. Y si aclara que quiere SOLO sus textos ("nada más", "solo esto", "sin agregar
nada"), los campos que no dictó van vacíos ("") — no le agregues frases de relleno.
...
- OJO con las tildes del voseo: el imperativo lleva tilde SOLO sin pronombre
  ("vení", "probá", "reservá", "pedí"); con pronombre pegado la PIERDE porque la
  palabra pasa a ser grave: "venite", "probala", "pedila", "contame", "sumate"
  (NUNCA "veníte", "probála", "pedíla"). Con dos pronombres vuelve como esdrújula:
  "probátela", "contámelo".
...
- "productoReal": true si el PROTAGONISTA visual de la escena es un plato NUESTRO
  (pizza, milanesa, empanadas, postre…). El sistema adjunta entonces una foto real
  del producto (del banco de la marca) como referencia, para que el plato generado
  se vea como el de verdad y no como comida genérica de stock. false si la escena
  no tiene comida propia como protagonista (ej: solo ambiente, partido, fachada).
    ...
    placas: {
      ...
      items: {
        ...
        properties: {
          ...
          productoReal: { type: 'boolean' },
        },
        required: ['titulo', 'acento', 'bajada', 'cta', 'lugar', 'estilo', 'escenaIA', 'escenaPistas', 'banderas', 'evento', 'ambienteReal', 'productoReal'],
...
// Tildes inequívocas que la IA a veces omite ("la semi se vive aca", 13 jul):
// para estas palabras NO existe versión válida sin tilde en el copy de la
// marca → fix determinístico, sin otra pasada de IA.
const TILDES_SEGURAS = {
  aca: 'acá', alla: 'allá', asi: 'así', aqui: 'aquí', ahi: 'ahí',
  despues: 'después', tambien: 'también', ademas: 'además', futbol: 'fútbol',
  miercoles: 'miércoles', sabado: 'sábado', menu: 'menú', jamon: 'jamón',
  limon: 'limón', proximo: 'próximo', proxima: 'próxima',
  pizzeria: 'pizzería', pizzerias: 'pizzerías',
};
// Tildes que SOBRAN en el voseo con clítico ("Veníte", 13 jul): el imperativo
// vos + pronombre es palabra grave terminada en vocal → SIEMPRE sin tilde.
const DESTILDES_VOSEO = {
  veníte: 'venite', probála: 'probala', probálo: 'probalo', pedíla: 'pedila',
  pedílo: 'pedilo', sumáte: 'sumate', contáme: 'contame', traéla: 'traela',
  traélo: 'traelo', mirála: 'mirala', llevála: 'llevala', llamáme: 'llamame',
};
function corregirTildes(s) {
  let out = String(s || '').replace(/\b[A-Za-z]+\b/g, (w) => {
    const fix = TILDES_SEGURAS[w.toLowerCase()];
    if (!fix) return w;
    if (w === w.toUpperCase()) return fix.toUpperCase();
    return w[0] === w[0].toUpperCase() ? fix[0].toUpperCase() + fix.slice(1) : fix;
  });
  for (const mal of Object.keys(DESTILDES_VOSEO)) {
    out = out.replace(new RegExp(mal, 'gi'), (w) => {
      const bien = DESTILDES_VOSEO[mal];
      if (w === w.toUpperCase()) return bien.toUpperCase();
      return w[0] === w[0].toUpperCase() ? bien[0].toUpperCase() + bien.slice(1) : bien;
    });
  }
  return out;
}
// El título y la bajada empiezan con mayúscula (regla de marca que la IA a
// veces saltea). El acento NO: la cursiva va en minúscula a propósito.
const mayusculaInicial = (s) => (s && /[a-záéíóúñ]/.test(s[0]) ? s[0].toLocaleUpperCase('es-ES') + s.slice(1) : s);
...
function sanearCopy(out) {
  ...
  (out.placas || []).forEach((p, i) => {
    ...
    if (p.titulo != null) p.titulo = mayusculaInicial(normalizarEspacios(corregirTildes(sanearTexto(p.titulo, 'el título', avisos))));
    if (p.acento != null) p.acento = normalizarEspacios(corregirTildes(sanearTexto(p.acento, 'el acento', avisos)));
    if (p.bajada != null) p.bajada = mayusculaInicial(normalizarEspacios(corregirTildes(sanearTexto(p.bajada, 'la bajada', avisos))));
    if (p.cta != null) p.cta = mayusculaInicial(normalizarEspacios(corregirTildes(sanearTexto(p.cta, 'el CTA', avisos))));
...
async function generarCopy(instruccion, formato) {
  ...
  const listo = await acortarLargos(sanearCopy(out));
  // El generador NO tiene internet: si la instrucción pide "averiguar" un dato,
  // el copy honesto sale SIN ese dato (nunca inventado) — pero hay que avisarlo
  // en la cara, no dejar al usuario esperando una fecha que no va a aparecer.
  if (/averigu|investig|busc\w*\s+(cu[aá]ndo|la fecha|el horario|la hora|qu[eé] d[ií]a)/i.test(String(instruccion))) {
    listo.avisos = (listo.avisos || []).concat(
      'No puedo buscar datos afuera (fechas, horas, rivales): lo que no esté en tu instrucción no aparece en la placa. Pasame el dato ("juegan el miércoles 15/7 a las 21:00") y va exacto.');
  }
  return listo;
}
...
"cta". Si la indicación DICTA un texto ("cambiá la bajada por: X", 'que diga "X"'),
ese texto va EXACTO, letra por letra — no lo reescribas ni lo "mejores".
...
async function llamarGemini(reqParts, aspecto, modelOverride) {
  if (!geminiDisponible()) {
    const e = new Error('Generación IA no disponible: falta GEMINI_API_KEY en el .env.');
    e.code = 'NO_KEY';
    throw e;
  }
  // modelOverride deja que un flujo puntual pida otro modelo (ej. portadas
  // editoriales con el PRO/Nano Banana) sin cambiar el global GEMINI_IMAGE_MODEL.
...
function promptPortadaGenerar(tema, color) {
  const col = String(color || '').trim() || 'warm amber';
  return 'Photorealistic editorial cover photograph, vertical 9:16 (1080x1920), cinematic ' +
    'dramatic moody lighting with a deep ' + col + '-toned palette. Theme: "' + tema + '".\n\n' +
    'STRICT COMPOSITION ZONES (respect them exactly):\n' +
    '- TOP 15% of the frame: keep it calm and uncluttered (a simple, darker area with ' +
    'no key detail) — a small brand header will sit here.\n' +
    '- CENTER band, from 20% to 55% of the height: this is where the main subject lives. ' +
    'Place the single strong subject (object, symbolic element, or an anonymous person ' +
    'with no recognizable frontal face) fully inside this central band, well framed and ' +
    'never cropped by the edges.\n' +
    '- BOTTOM 40% of the frame: keep it noticeably DARKER, emptier and uncluttered, with ' +
    'clean negative space and a smooth gradient fading toward the bottom — a large title ' +
    'will be overlaid here, so nothing important must be in this lower area.\n\n' +
    'Balanced, premium, editorial feel. No commercial brands or logos. ' +
    'ABSOLUTELY NO TEXT of any kind: no letters, words, numbers, captions, titles, ' +
    'signatures, watermarks, labels, frames or borders anywhere in the image.';
}
...
// Dibuja un TÍTULO EXACTO sobre la portada ya generada/limpiada. El texto lo
// pone el sistema con sharp (Montserrat bold, fuente de marca), NUNCA la IA:
// así el título es literal y nunca se inventa. Un scrim inferior lo hace legible
// sobre cualquier foto. Sin título, la portada sale limpia como antes.
async function overlayTituloPortada(imgBuf, titulo) {
  const t = String(titulo || '').trim();
  if (!t) return imgBuf;
  const W = 1080, H = 1920;
  const lines = wrap(t.toUpperCase(), 13).slice(0, 3);
  const longest = lines.reduce((m, l) => Math.max(m, l.length), 0);
  const fs = longest > 11 ? 82 : (longest > 8 ? 94 : 108);
  const lh = Math.round(fs * 1.05);
  const bottom = 1730; // margen inferior seguro: la UI del reel tapa lo de más abajo
  const first = bottom - (lines.length - 1) * lh;
  const tspans = lines.map((l, i) =>
    '<text x="' + (W / 2) + '" y="' + (first + i * lh) + '" text-anchor="middle" ' +
    'font-family="' + SANS + '" font-weight="800" font-size="' + fs + '" letter-spacing="1" ' +
    'fill="#fff">' + escXml(l) + '</text>').join('');
  const scrimTop = Math.min(first - fs - 60, 1040);
  const svg = '<svg xmlns="http://www.w3.org/2000/svg" width="' + W + '" height="' + H + '">' +
    '<defs><linearGradient id="s" x1="0" y1="0" x2="0" y2="1">' +
    '<stop offset="0" stop-color="#000" stop-opacity="0"/>' +
    '<stop offset=".55" stop-color="#000" stop-opacity=".55"/>' +
    '<stop offset="1" stop-color="#000" stop-opacity=".85"/></linearGradient></defs>' +
    '<rect x="0" y="' + scrimTop + '" width="' + W + '" height="' + (H - scrimTop) + '" fill="url(#s)"/>' +
    tspans + '</svg>';
  return sharp(imgBuf).composite([{ input: Buffer.from(svg), top: 0, left: 0 }])
    .jpeg({ quality: 92, mozjpeg: true }).toBuffer();
}

// Header de marca en la franja superior de la portada: el wordmark blanco real
// centrado arriba (con la sombra suave de prepararLogo para que lea siempre).
// Convierte la banda oscura reservada por el prompt en un header editorial de
// verdad, no en un borde negro vacío.
async function overlayHeaderPortada(imgBuf) {
  const logo = await prepararLogo('wordmark-blanco', 1080, 1920, 'arriba');
  if (!logo) return imgBuf;
  return sharp(imgBuf).composite(logo.capas).jpeg({ quality: 92, mozjpeg: true }).toBuffer();
}

// Título sobre una captura limpia, en 5 DISEÑOS de marca (basados en las placas
// de referencia reales del Drive). El campo admite "Principal | secundario".
// Todo por CÓDIGO con tinta real medida (trim) y sombra suave: texto exacto,
// imagen intacta. Diseños:
//   milanesa — serif blanca + cursiva dorada abajo (ref "Milanesa Buenos Aires")
//   boadilla — MAYÚSCULAS sans + cursiva dorada grande (ref "Hoy en Boadilla")
//   firma    — cursiva dorada GIGANTE + kicker chico arriba (la firma de la casa)
//   dorado   — MAYÚSCULAS blancas con la última palabra en dorado (eco portadas)
//   arriba   — como boadilla pero el texto va ARRIBA (deja el plato libre abajo)
// Todos con wordmark real centrado abajo y scrim según el diseño.
const DISENOS_CAPTURA = ['milanesa', 'boadilla', 'firma', 'dorado', 'arriba', 'marco', 'sello', 'neon', 'cinta', 'vidrio', 'poster'];

async function overlayTituloCaptura(imgBuf, titulo, diseno) {
  const raw = String(titulo || '').trim();
  if (!raw) return imgBuf;
  const d = DISENOS_CAPTURA.includes(diseno) ? diseno : 'milanesa';
  const [tTxt, sec] = raw.split('|').map((s) => String(s || '').trim());
  const W = 1080, H = 1920;

  // Renderiza un texto, recortado a tinta real (trim), con sombra suave
  // (silueta negra difuminada, como el logo) para que lea sobre foto clara.
  const pieza = async (txt, fill, font, fsz, ls) => {
    const wEst = Math.ceil(txt.length * fsz * 1.25) + 180;
    const hEst = Math.ceil(fsz * 2.2);
    const svg = '<svg xmlns="http://www.w3.org/2000/svg" width="' + wEst + '" height="' + hEst + '">' +
      '<text x="16" y="' + Math.round(fsz * 1.32) + '" font-family="' + font + '" ' +
      (font === SERIF || font === BRUSH ? '' : 'font-weight="bold" ') +
      (ls ? 'letter-spacing="' + ls + '" ' : '') +
      'font-size="' + fsz + '" fill="' + fill + '">' + escXml(txt) + '</text></svg>';
    const p = await sharp(Buffer.from(svg)).png().trim({ threshold: 5 }).toBuffer({ resolveWithObject: true });
    const alpha = await sharp(p.data).extractChannel('alpha').toBuffer();
    const silueta = await sharp({
      create: { width: p.info.width, height: p.info.height, channels: 3, background: { r: 0, g: 0, b: 0 } },
    }).joinChannel(alpha).png().toBuffer();
    const sombra = await sharp(silueta)
      .extend({ top: 14, bottom: 14, left: 14, right: 14, background: { r: 0, g: 0, b: 0, alpha: 0 } })
      .blur(9).png().toBuffer();
    return { buf: p.data, w: p.info.width, h: p.info.height, sombra };
  };

  // Filas del bloque: cada fila = lista de piezas en línea (para "dorado", que
  // parte la última línea en tramo blanco + palabra dorada).
  const filas = [];
  const lines = wrap(tTxt, 14).slice(0, 3);
  const longest = lines.reduce((m, l) => Math.max(m, l.length), 0);

  // ---- Diseños "libres": layout propio, retorno directo ----
  if (d === 'marco' || d === 'sello' || d === 'neon' || d === 'cinta' || d === 'vidrio' || d === 'poster') {
    const logo = await prepararLogo('wordmark-blanco', W, H, 'abajo', 240);
    const logoTop = logo ? logo.capas[1].top : H - 390;
    const capas = [];
    let scrimExtra = '';
    const defs = '<defs>' +
      '<linearGradient id="b" x1="0" y1="0" x2="0" y2="1">' +
      '<stop offset="0" stop-color="#000" stop-opacity="0"/>' +
      '<stop offset=".45" stop-color="#000" stop-opacity=".5"/>' +
      '<stop offset="1" stop-color="#000" stop-opacity=".85"/></linearGradient>' +
    ...
    if (d === 'marco') {
      // MARCO EDITORIAL: doble marco dorado fino + bloque serif alineado a la
      // IZQUIERDA con una barrita dorada arriba (tapa de menú premium).
      const mx = 116;
      const fs = longest <= 8 ? 116 : (longest <= 12 ? 98 : 84);
      const piezas = [];
      for (const l of wrap(tTxt, 16).slice(0, 3)) piezas.push(await pieza(l, '#fff', SERIF, fs));
      if (sec) {
        let c = await pieza(sec, '#F5C66B', BRUSH, Math.round(fs * 1.1));
        if (c.w > W - mx - 90) c = await pieza(sec, '#F5C66B', BRUSH, Math.max(56, Math.floor(fs * 1.1 * ((W - mx - 90) / c.w))));
        piezas.push(c);
      }
      const gapR = 30;
      const totalH = piezas.reduce((s, p) => s + p.h, 0) + gapR * (piezas.length - 1);
      let y = logoTop - 64 - totalH;
      const barY = y - 44;
      for (const p of piezas) {
        capas.push({ input: p.sombra, left: mx - 14, top: y - 14 + 3 });
        capas.push({ input: p.buf, left: mx, top: y });
        y += p.h + gapR;
      }
      scrimExtra =
        '<rect x="0" y="' + (barY - 120) + '" width="' + W + '" height="' + (H - barY + 120) + '" fill="url(#b)"/>' +
        '<rect x="' + mx + '" y="' + barY + '" width="120" height="6" fill="#F5C66B"/>' +
        '<rect x="44" y="44" width="' + (W - 88) + '" height="' + (H - 88) + '" fill="none" stroke="' + GOLD + '" stroke-width="3"/>' +
        '<rect x="60" y="60" width="' + (W - 120) + '" height="' + (H - 120) + '" fill="none" stroke="#F5C66B" stroke-opacity=".55" stroke-width="1"/>';
    } else if (d === 'sello') {
      // SELLO DE LA CASA: doble anillo dorado con el título adentro y tres
      // puntitos de ornamento (sello estampado de trattoria).
      const r = 285, cx = W / 2;
      const cy = logoTop - 80 - r;
      const up = wrap(tTxt.toUpperCase(), 12).slice(0, 2);
      const lg = up.reduce((m, l) => Math.max(m, l.length), 0);
      const fs = lg <= 8 ? 92 : (lg <= 11 ? 80 : 68);
      const piezas = [];
      for (const l of up) {
        let p = await pieza(l, '#fff', SANS, fs, 2);
        if (p.w > r * 2 - 150) p = await pieza(l, '#fff', SANS, Math.max(44, Math.floor(fs * ((r * 2 - 150) / p.w))), 2);
        piezas.push(p);
      }
      let c = null;
      if (sec) {
        c = await pieza(sec, '#F5C66B', BRUSH, Math.round(fs * 1.15));
        if (c.w > r * 2 - 120) c = await pieza(sec, '#F5C66B', BRUSH, Math.max(50, Math.floor(fs * 1.15 * ((r * 2 - 120) / c.w))));
      }
      const gapR = 26;
      const totalH = piezas.reduce((s, p) => s + p.h, 0) + (c ? c.h + gapR : 0) + gapR * (piezas.length - 1) + 46;
      let y = Math.round(cy - totalH / 2) + 46;
      scrimExtra =
        '<rect x="0" y="' + (cy - r - 60) + '" width="' + W + '" height="' + (r * 2 + 120) + '" fill="url(#r)"/>' +
        '<rect x="0" y="' + (logoTop - 90) + '" width="' + W + '" height="' + (H - logoTop + 90) + '" fill="url(#b)"/>' +
        '<circle cx="' + cx + '" cy="' + cy + '" r="' + r + '" fill="none" stroke="' + GOLD + '" stroke-width="3"/>' +
        '<circle cx="' + cx + '" cy="' + cy + '" r="' + (r - 16) + '" fill="none" stroke="#F5C66B" stroke-opacity=".7" stroke-width="1.5"/>' +
        '<circle cx="' + (cx - 26) + '" cy="' + (y - 34) + '" r="5" fill="#F5C66B"/>' +
        '<circle cx="' + cx + '" cy="' + (y - 34) + '" r="5" fill="#F5C66B"/>' +
        '<circle cx="' + (cx + 26) + '" cy="' + (y - 34) + '" r="5" fill="#F5C66B"/>';
      for (const p of piezas) {
        capas.push({ input: p.sombra, left: Math.round((W - p.w) / 2) - 14, top: y - 14 + 3 });
        capas.push({ input: p.buf, left: Math.round((W - p.w) / 2), top: y });
        y += p.h + gapR;
      }
      if (c) {
        capas.push({ input: c.sombra, left: Math.round((W - c.w) / 2) - 14, top: y - 14 + 3 });
        capas.push({ input: c.buf, left: Math.round((W - c.w) / 2), top: y });
      }
    } else if (d === 'cinta') {
      // CINTA EDITORIAL: banda espresso inclinada con filos dorados y el
      // texto adentro (máxima legibilidad sobre cualquier foto).
      const ang = -3.5;
      const up = tTxt.toUpperCase();
      let fsT = up.length <= 8 ? 118 : (up.length <= 12 ? 100 : 84);
      if (up.length * 0.74 * fsT > 920) fsT = Math.max(54, Math.floor(920 / (up.length * 0.74)));
      const fsC = sec ? Math.round(fsT * 1.05) : 0;
      const bandH = 52 * 2 + Math.round(fsT * 0.95) + (sec ? Math.round(fsC * 1.15) + 14 : 0);
      const cy = logoTop - 130 - Math.round(bandH / 2);
      const rectY = cy - Math.round(bandH / 2);
      const g0 = '<g transform="rotate(' + ang + ' 540 ' + cy + ')">';
      // sombra de la banda (silueta difuminada)
      const shSvg = '<svg xmlns="http://www.w3.org/2000/svg" width="' + W + '" height="' + H + '">' +
        g0 + '<rect x="-220" y="' + rectY + '" width="1520" height="' + bandH + '" fill="#000" fill-opacity=".6"/></g></svg>';
      const sh = await sharp(Buffer.from(shSvg)).png().blur(16).toBuffer();
      capas.push({ input: sh, left: 0, top: 6 });
      const ty1 = rectY + 52 + Math.round(fsT * 0.82);
      const ty2 = ty1 + Math.round(fsC * 0.95) + 14;
      const bandSvg = '<svg xmlns="http://www.w3.org/2000/svg" width="' + W + '" height="' + H + '">' + g0 +
        '<rect x="-220" y="' + rectY + '" width="1520" height="' + bandH + '" fill="#171310" fill-opacity=".96"/>' +
        '<rect x="-220" y="' + (rectY + 12) + '" width="1520" height="2.5" fill="' + GOLD + '"/>' +
        '<rect x="-220" y="' + (rectY + bandH - 15) + '" width="1520" height="2.5" fill="' + GOLD + '"/>' +
        '<text x="540" y="' + ty1 + '" text-anchor="middle" font-family="' + SANS + '" font-weight="bold" ' +
        'font-size="' + fsT + '" letter-spacing="3" fill="#fff">' + escXml(up) + '</text>' +
        (sec ? '<text x="540" y="' + ty2 + '" text-anchor="middle" font-family="' + BRUSH + '" ' +
          'font-size="' + fsC + '" fill="#F5C66B">' + escXml(sec) + '</text>' : '') +
        '</g></svg>';
      capas.push({ input: Buffer.from(bandSvg), left: 0, top: 0 });
      scrimExtra = '<rect x="0" y="' + (logoTop - 90) + '" width="' + W + '" height="' + (H - logoTop + 90) + '" fill="url(#b)"/>';
    } else if (d === 'vidrio') {
      // TARJETA DE VIDRIO: panel esmerilado (la propia foto desenfocada) con
      // borde dorado y brillo; el texto vive adentro. Moderno y premium.
      const fsV = longest <= 8 ? 108 : (longest <= 12 ? 92 : 78);
      const piezasV = [];
      for (const l of wrap(tTxt, 16).slice(0, 3)) piezasV.push(await pieza(l, '#fff', SERIF, fsV));
      if (sec) {
        let cV = await pieza(sec, '#F5C66B', BRUSH, Math.round(fsV * 1.15));
        if (cV.w > 750) cV = await pieza(sec, '#F5C66B', BRUSH, Math.max(46, Math.floor(fsV * 1.15 * (750 / cV.w))));
        piezasV.push(cV);
      }
      const gapV = 26;
      const contentH = piezasV.reduce((s, p) => s + p.h, 0) + gapV * (piezasV.length - 1);
      const cardW = Math.min(940, Math.max(...piezasV.map((p) => p.w)) + 150);
      const cardH = contentH + 130;
      const cardX = Math.round((W - cardW) / 2);
      const cardY = logoTop - 64 - cardH;
      // esmerilado: recorte de la MISMA foto, desenfocado + oscurecido + redondeado
      const region = await sharp(imgBuf).extract({ left: cardX, top: cardY, width: cardW, height: cardH })
        .blur(24).modulate({ brightness: 0.68 }).png().toBuffer();
      const maskR = Buffer.from('<svg xmlns="http://www.w3.org/2000/svg" width="' + cardW + '" height="' + cardH + '">' +
        '<rect width="' + cardW + '" height="' + cardH + '" rx="34" fill="#fff"/></svg>');
      const frost = await sharp(region).composite([{ input: maskR, blend: 'dest-in' }]).png().toBuffer();
      const shC = await sharp(Buffer.from('<svg xmlns="http://www.w3.org/2000/svg" width="' + (cardW + 90) + '" height="' + (cardH + 90) + '">' +
        '<rect x="45" y="45" width="' + cardW + '" height="' + cardH + '" rx="34" fill="#000" fill-opacity=".55"/></svg>'))
        .png().blur(20).toBuffer();
      capas.push({ input: shC, left: cardX - 45, top: cardY - 35 });
      capas.push({ input: frost, left: cardX, top: cardY });
      capas.push({
        input: Buffer.from('<svg xmlns="http://www.w3.org/2000/svg" width="' + cardW + '" height="' + cardH + '">' +
          '<rect x="1.5" y="1.5" width="' + (cardW - 3) + '" height="' + (cardH - 3) + '" rx="33" fill="none" stroke="' + GOLD + '" stroke-width="2.5"/>' +
          '<rect x="18" y="10" width="' + (cardW - 36) + '" height="1.5" fill="#fff" fill-opacity=".3"/></svg>'),
        left: cardX, top: cardY,
      });
      let yV = cardY + 65;
      for (const p of piezasV) {
        capas.push({ input: p.buf, left: Math.round((W - p.w) / 2), top: yV });
        yV += p.h + gapV;
      }
      scrimExtra = '<rect x="0" y="' + (logoTop - 90) + '" width="' + W + '" height="' + (H - logoTop + 90) + '" fill="url(#b)"/>';
    } else if (d === 'poster') {
      // PÓSTER TIPOGRÁFICO: mayúsculas enormes + la última palabra en cursiva
      // dorada GIGANTE montada encima, apenas inclinada y con glow.
      const words = tTxt.split(/\s+/);
      const lastW = words.pop();
      const capsTxt = words.join(' ').toUpperCase();
      let capsP = null;
      if (capsTxt) {
        let fsP = 190;
        capsP = await pieza(capsTxt, '#fff', SANS, fsP, 2);
        if (capsP.w > W - 170) { fsP = Math.max(76, Math.floor(fsP * ((W - 170) / capsP.w))); capsP = await pieza(capsTxt, '#fff', SANS, fsP, 2); }
      }
      let fsX = 265;
      let cur = await pieza(lastW, '#F5C66B', BRUSH, fsX);
      if (cur.w > W - 150) { fsX = Math.max(96, Math.floor(fsX * ((W - 150) / cur.w))); cur = await pieza(lastW, '#F5C66B', BRUSH, fsX); }
      const curRot = await sharp(cur.buf).rotate(-5, { background: { r: 0, g: 0, b: 0, alpha: 0 } })
        .png().toBuffer({ resolveWithObject: true });
      const alphaP = await sharp(curRot.data).extractChannel('alpha').toBuffer();
      const oroP = await sharp({ create: { width: curRot.info.width, height: curRot.info.height, channels: 3, background: { r: 224, g: 150, b: 60 } } })
        .joinChannel(alphaP).png().toBuffer();
      const glowP = await sharp(oroP).extend({ top: 40, bottom: 40, left: 40, right: 40, background: { r: 0, g: 0, b: 0, alpha: 0 } })
        .blur(20).png().toBuffer();
      let kickP = null;
      if (sec) kickP = await pieza(sec.toUpperCase(), '#fff', SANS, 42, 6);
      const solapa = capsP ? Math.round(curRot.info.height * 0.42) : 0;
      const bottomP = logoTop - 84;
      const curTop = bottomP - curRot.info.height;
      const capsBottom = capsP ? curTop + solapa : curTop;
      const capsTop = capsP ? capsBottom - capsP.h : curTop;
      scrimExtra = '<rect x="0" y="' + ((kickP ? capsTop - 60 - kickP.h : capsTop) - 160) + '" width="' + W + '" height="' + (H - capsTop + 220) + '" fill="url(#b)"/>';
      if (kickP) {
        const kTop = capsTop - 52 - kickP.h;
        capas.push({ input: kickP.sombra, left: Math.round((W - kickP.w) / 2) - 14, top: kTop - 14 + 3 });
        capas.push({ input: kickP.buf, left: Math.round((W - kickP.w) / 2), top: kTop });
      }
      if (capsP) {
        const cx0 = Math.round((W - capsP.w) / 2);
        capas.push({ input: capsP.sombra, left: cx0 - 14, top: capsTop - 14 + 3 });
        capas.push({ input: capsP.sombra, left: cx0 - 14, top: capsTop - 14 + 3 });
        capas.push({ input: capsP.buf, left: cx0, top: capsTop });
      }
      const cxr = Math.round((W - curRot.info.width) / 2);
      capas.push({ input: glowP, left: cxr - 40, top: curTop - 40 });
      capas.push({ input: curRot.data, left: cxr, top: curTop });
    } else {
      // NEÓN CÁLIDO: la cursiva gigante con glow dorado (cartel de neón de
      // barrio) + kicker chico en mayúsculas arriba.
      let fsN = 230;
      let p = await pieza(tTxt, '#FFE9C4', BRUSH, fsN);
      if (p.w > W - 150) { fsN = Math.max(90, Math.floor(fsN * ((W - 150) / p.w))); p = await pieza(tTxt, '#FFE9C4', BRUSH, fsN); }
      const alphaN = await sharp(p.buf).extractChannel('alpha').toBuffer();
      const oro = await sharp({ create: { width: p.w, height: p.h, channels: 3, background: { r: 224, g: 150, b: 60 } } })
        .joinChannel(alphaN).png().toBuffer();
      const ext = { top: 46, bottom: 46, left: 46, right: 46, background: { r: 0, g: 0, b: 0, alpha: 0 } };
      const glowBig = await sharp(oro).extend(ext).blur(26).png().toBuffer();
      const glowSm = await sharp(oro).extend(ext).blur(9).png().toBuffer();
      let kick = null;
      if (sec) kick = await pieza(sec.toUpperCase(), '#fff', SANS, 44, 6);
      const bottomY = logoTop - 90;
      const pTop = bottomY - p.h;
      const px = Math.round((W - p.w) / 2);
      scrimExtra = '<rect x="0" y="' + (pTop - 200) + '" width="' + W + '" height="' + (H - pTop + 200) + '" fill="url(#b)"/>';
      if (kick) {
        const kTop = pTop - 54 - kick.h;
        capas.push({ input: kick.sombra, left: Math.round((W - kick.w) / 2) - 14, top: kTop - 14 + 3 });
        capas.push({ input: kick.buf, left: Math.round((W - kick.w) / 2), top: kTop });
      }
      capas.push({ input: glowBig, left: px - 46, top: pTop - 46 });
      capas.push({ input: glowSm, left: px - 46, top: pTop - 46 });
      capas.push({ input: p.buf, left: px, top: pTop });
    }

    const scrim = '<svg xmlns="http://www.w3.org/2000/svg" width="' + W + '" height="' + H + '">' + defs + scrimExtra + '</svg>';
    const conTexto = await sharp(imgBuf)
      .composite([{ input: Buffer.from(scrim), top: 0, left: 0 }, ...capas])
      .png().toBuffer();
    return sharp(conTexto).composite(logo ? logo.capas : [])
      .jpeg({ quality: 92, mozjpeg: true }).toBuffer();
  }

  if (d === 'milanesa') {
    const fs = longest <= 8 ? 132 : (longest <= 11 ? 116 : 100);
    for (const l of lines) filas.push([await pieza(l, '#fff', SERIF, fs)]);
    if (sec) filas.push([await pieza(sec, GOLD, BRUSH, Math.round(fs * 1.2))]);
  } else if (d === 'boadilla' || d === 'arriba') {
    const up = lines.map((l) => l.toUpperCase());
    const esA = d === 'arriba';
    // 'arriba' (el establecido): título bien GRANDE para que golpee; se ajusta
    // solo al ancho si la línea es larga.
    const fs = esA
      ? (longest <= 8 ? 132 : (longest <= 11 ? 114 : 96))
      : (longest <= 8 ? 104 : (longest <= 11 ? 92 : 80));
    for (const l of up) {
      let p = await pieza(l, '#fff', SANS, fs, esA ? 2 : 3);
      if (p.w > W - 120) p = await pieza(l, '#fff', SANS, Math.max(56, Math.floor(fs * ((W - 120) / p.w))), esA ? 2 : 3);
      filas.push([p]);
    }
    if (sec) {
      // En 'arriba' la cursiva es PROTAGONISTA: bien grande (ajustada al ancho
      // para que nunca se corte) y en el dorado claro de marca. En 'boadilla'
      // sigue de acompañamiento.
      const fszC = d === 'arriba' ? Math.round(fs * 2.3) : Math.round(fs * 1.5);
      const colC = d === 'arriba' ? '#F5C66B' : GOLD;
      let c = await pieza(sec, colC, BRUSH, fszC);
      const maxW = W - 140;
      if (c.w > maxW) c = await pieza(sec, colC, BRUSH, Math.max(64, Math.floor(fszC * (maxW / c.w))));
      filas.push([c]);
    }
  } else if (d === 'firma') {
    if (sec) filas.push([await pieza(sec.toUpperCase(), '#fff', SANS, 42, 6)]);
    const fsB = longest <= 10 ? 185 : (longest <= 14 ? 160 : 135);
    for (const l of wrap(tTxt, 16).slice(0, 2)) filas.push([await pieza(l, GOLD, BRUSH, fsB)]);
  } else { // dorado
    const up = lines.map((l) => l.toUpperCase());
    const fs = longest <= 8 ? 112 : (longest <= 11 ? 98 : 84);
    for (let i = 0; i < up.length; i++) {
      if (i < up.length - 1) { filas.push([await pieza(up[i], '#fff', SANS, fs, 2)]); continue; }
      const palabras = up[i].split(/\s+/);
      const ultima = palabras.pop();
      const fila = [];
      if (palabras.length) fila.push(await pieza(palabras.join(' '), '#fff', SANS, fs, 2));
      fila.push(await pieza(ultima, GOLD, SANS, fs, 2));
      filas.push(fila);
    }
    if (sec) filas.push([await pieza(sec, GOLD, BRUSH, Math.round(fs * 1.3))]);
  }

  // MÁRGENES SEGUROS (medidos en las placas de referencia y por la UI del reel):
  // el logo termina a ~240px del borde inferior; el texto del diseño 'arriba'
  // arranca a ~290px del techo. Nada vive pegado a los bordes.
  const logo = await prepararLogo('wordmark-blanco', W, H, 'abajo', 240);
  const logoTop = logo ? logo.capas[1].top : H - 240 - 150;

  // Apilado del bloque: arriba (diseño 'arriba') o encima del logo (el resto).
  const lhBase = Math.max(...filas.map((f) => Math.max(...f.map((p) => p.h))));
  const lh = Math.round(lhBase * 1.28);
  const gap = 26;
  const capas = [];
  const esArriba = d === 'arriba';
  const bottomTexto = logoTop - 56; // última fila apoya justo sobre el logo
  let yCursor = 290; // 'arriba': apilado compacto fila a fila (no grilla uniforme)
  for (let i = 0; i < filas.length; i++) {
    const f = filas[i];
    const totalW = f.reduce((s, p) => s + p.w, 0) + gap * (f.length - 1);
    const rowH = Math.max(...f.map((p) => p.h));
    const rowBottom = esArriba
      ? yCursor + rowH
      : bottomTexto - (filas.length - 1 - i) * lh;
    let x = Math.round((W - totalW) / 2);
    for (const p of f) {
      const top = rowBottom - rowH + Math.round((rowH - p.h) / 2);
      capas.push({ input: p.sombra, left: x - 14, top: top - 14 + 3 });
      // 'arriba': sombra DOBLE = más contraste, lee sobre cualquier fondo.
      if (esArriba) capas.push({ input: p.sombra, left: x - 14, top: top - 14 + 3 });
      capas.push({ input: p.buf, left: x, top });
      x += p.w + gap;
    }
    if (esArriba) yCursor = rowBottom + 42;
  }

  // Scrims: sostienen el texto y el logo sobre cualquier foto, sin tapar el plato.
  const altoBloque = filas.length * lh + 110;
  const scrim = '<svg xmlns="http://www.w3.org/2000/svg" width="' + W + '" height="' + H + '">' +
    ...
    (esArriba
      ? '<rect x="0" y="0" width="' + W + '" height="' + (yCursor + 90) + '" fill="url(#t)"/>' +
        '<rect x="0" y="' + (logoTop - 90) + '" width="' + W + '" height="' + (H - logoTop + 90) + '" fill="url(#b)"/>'
      : '<rect x="0" y="' + (bottomTexto - altoBloque) + '" width="' + W + '" height="' + (H - (bottomTexto - altoBloque)) + '" fill="url(#b)"/>') +
    '</svg>';

  const conTexto = await sharp(imgBuf)
    .composite([{ input: Buffer.from(scrim), top: 0, left: 0 }, ...capas])
    .png().toBuffer();
  // Wordmark real centrado abajo, dentro del margen seguro.
  return sharp(conTexto).composite(logo ? logo.capas : [])
    .jpeg({ quality: 92, mozjpeg: true }).toBuffer();
}

// Título "DISEÑADO POR IA": el modelo PRO (el mismo de las portadas editoriales,
// que escribe perfecto) limpia el frame Y diseña la tipografía INTEGRADA a la
// foto como tapa de revista — luz, profundidad, materiales. Red de seguridad:
// verificación letra por letra con Claude vision + 1 reintento con feedback, y
// el logo real SIEMPRE pegado por código. Máxima calidad visual con texto exacto.
const PORTADA_IA_VERIF_SCHEMA = {
  type: 'object',
  properties: {
    ok: { type: 'boolean' },
    errores: { type: 'array', items: { type: 'string' } },
    leido: { type: 'string' },
    textoTopPct: { type: 'number', description: 'Borde superior del texto más alto, como % de la altura (0 = borde superior de la imagen).' },
    textoBottomPct: { type: 'number', description: 'Borde inferior del texto más bajo, como % de la altura.' },
    tocaBordeLateral: { type: 'boolean', description: 'true si alguna letra toca el borde izquierdo o derecho.' },
  },
  required: ['ok', 'errores', 'leido', 'textoTopPct', 'textoBottomPct', 'tocaBordeLateral'],
  additionalProperties: false,
};

// Estilos de la casa para portadas sobre captura (fijados con el cliente,
// jul 2026). Coda de legibilidad compartida: sombra profunda por letra +
// oscurecer el fondo detrás del texto (el dorado se perdía sobre tonos claros).
const PORTADA_IA_LEGIBILIDAD =
  'LEGIBILITY IS CRITICAL: paint a soft believable dark gradient into the photo behind ' +
  'the text area (like cinematic grading), and give every letter a deep soft drop shadow ' +
  'so it separates cleanly from the scene. If any letters sit over similar warm tones, ' +
  'darken the background behind them further. Every letter must be instantly readable ' +
  'from a phone. IMPORTANT: any darkening/grading fades out smoothly BEFORE reaching ' +
  'the top and bottom edges of the canvas — the outermost strips must keep the original ' +
  'photo grading untouched (this crop is blended back into a taller cover).';
// Sin segunda línea del usuario NO se inventa una: el brief lo dice explícito
// (bug real del 12 jul: "FLAN MIXTO" solo → la IA agregó una bajada propia).
const PORTADA_IA_SIN_SEC =
  'The title is the ONLY text on the cover — do NOT add any secondary line, subtitle ' +
  'or slogan. ';
// Los briefs son funciones de la línea secundaria (existe o no).
const PORTADA_IA_BRIEFS = {
  firma: (sec) =>
    'The main title as a BIG gold (#F5C66B) handwritten script, like a chef signature — ' +
    'sweeping but ALWAYS inside the safe box, never touching the image edges. COPY the ' +
    'exact letterforms of the brand script shown in the typography reference sheet. ' +
    (sec
      ? 'The secondary line below in clean white geometric sans (style of "Montserrat"), ' +
        'uppercase, generously letterspaced, LARGE enough to read easily on a phone. '
      : PORTADA_IA_SIN_SEC) +
    PORTADA_IA_LEGIBILIDAD,
  poster: (sec) =>
    'Giant ultra-bold condensed uppercase poster typography, the words stacked in rows ' +
    '(or one row if it fits the photo better): ONE key word in warm gold (#F5C66B), the ' +
    'rest in pure white. The dish may slightly overlap the letters for depth (occlusion), ' +
    'but at least 80% of every word stays visible. ' +
    (sec
      ? 'The secondary line in WHITE in the brand script: COPY its letterforms from the ' +
        'typography reference sheet EXACTLY, keeping its THIN elegant pen strokes — do ' +
        'NOT thicken it into a rounded marker script. '
      : PORTADA_IA_SIN_SEC) +
    PORTADA_IA_LEGIBILIDAD,
};

// La cursiva de la marca (Abuget) no se puede describir con palabras — pintada
// "de oído" sale una brush genérica (feedback del cliente, 12 jul 2026). Se
// renderizan los textos EXACTOS con la Abuget real (fontconfig → fonts/ del
// proyecto) y se pasan como imagen de referencia para que el PRO copie trazos.
async function muestraScriptMarca(tTxt, sec) {
  const linea = async (txt, fs) => {
    const svg = '<svg xmlns="http://www.w3.org/2000/svg" width="3000" height="' + Math.round(fs * 3.4) + '">' +
      '<text x="400" y="' + Math.round(fs * 2) + '" font-family="' + BRUSH + '" font-size="' + fs + '" fill="#111">' +
      escXml(txt) + '</text></svg>';
    const png = await sharp(Buffer.from(svg)).png().toBuffer();
    return sharp(png).trim({ threshold: 5 }).toBuffer();
  };
  const filas = [];
  for (const [txt, fs] of [[tTxt, 180], [sec, 130]]) {
    if (!txt) continue;
    const img = await linea(txt, fs);
    filas.push({ img, meta: await sharp(img).metadata() });
  }
  if (!filas.length) return null;
  const w = Math.max(...filas.map((f) => f.meta.width)) + 160;
  let y = 80;
  const capas = filas.map((f) => { const c = { input: f.img, left: 80, top: y }; y += f.meta.height + 70; return c; });
  return sharp({ create: { width: w, height: y + 10, channels: 3, background: '#ffffff' } })
    .composite(capas).jpeg({ quality: 92 }).toBuffer();
}

// Ventana del grid: el tile 3:4 del perfil de IG muestra y∈[240,1680] de la
// portada 9:16. La tipografía se diseña SOLO dentro de esa ventana (el PRO
// ignora márgenes numéricos y copia guías dibujadas — probado 12 jul 2026),
// así que el recorte hace estructuralmente imposible que el texto quede fuera.
const PORTADA_VENTANA = { top: 240, height: 1440 };
// Máscara de fundido (180px arriba / 90px abajo) para reintegrar la ventana
// diseñada a la portada completa sin costura visible. El fundido superior es
// más largo porque ahí el modelo suele oscurecer detrás del texto y en fondos
// claros la transición corta se veía como banda (milanesa, 13 jul).
const PORTADA_VENTANA_MASK =
  '<svg xmlns="http://www.w3.org/2000/svg" width="1080" height="1440"><defs>' +
  '<linearGradient id="f" x1="0" y1="0" x2="0" y2="1">' +
  '<stop offset="0" stop-color="#fff" stop-opacity="0"/>' +
  '<stop offset="0.125" stop-color="#fff" stop-opacity="1"/>' +
  '<stop offset="0.9375" stop-color="#fff" stop-opacity="1"/>' +
  '<stop offset="1" stop-color="#fff" stop-opacity="0"/>' +
  '</linearGradient></defs>' +
  '<rect width="1080" height="1440" fill="url(#f)"/></svg>';

async function disenoPortadaIA(frameBuf, titulo, estilo) {
  const [tTxt, sec] = String(titulo || '').split('|').map((s) => String(s || '').trim());
  const frame = await sharp(frameBuf).rotate()
    .resize(1080, 1920, { fit: 'cover', position: 'centre' })
    .jpeg({ quality: 90 }).toBuffer();
  // Gemini a veces devuelve vacío ("no devolvió una imagen") de forma
  // transitoria: un reintento simple lo resuelve casi siempre.
  const conReintento = async (fn) => { try { return await fn(); } catch (e) { return fn(); } };
  // PASADA 1 — limpiar el frame completo (mismo prompt validado del modo
  // limpiar sin título; modelo flash alcanza y es rápido).
  const limpioRaw = await conReintento(() => llamarGemini([
    { inlineData: { mimeType: 'image/jpeg', data: frame.toString('base64') } },
    { text: PROMPT_PORTADA_LIMPIAR },
  ], '9:16'));
  const limpio = await sharp(limpioRaw).resize(1080, 1920, { fit: 'cover', position: 'centre' })
    .jpeg({ quality: 92 }).toBuffer();
  const ventanaBase = await sharp(limpio)
    .extract({ left: 0, top: PORTADA_VENTANA.top, width: 1080, height: PORTADA_VENTANA.height })
    .jpeg({ quality: 92 }).toBuffer();
  // Dirección de arte de la tipografía: configurable (permite variantes); el
  // default ancla al manual de marca (Abril Fatface / Montserrat / Abuget) —
  // el PRO pinta las letras, así que se nombran + describen los estilos.
  const estiloTxt = (typeof estilo === 'function' ? estilo(sec) : String(estilo || '').trim()) ||
    ('Follow the brand typography: the main title in an elegant high-contrast Didone serif ' +
    'in the style of "Abril Fatface", OR in a clean geometric sans in the style of ' +
    '"Montserrat" (bold, uppercase, letterspaced). ' +
    (sec
      ? 'The secondary line may use the brand handwritten script (copy it from the ' +
        'typography reference sheet). '
      : PORTADA_IA_SIN_SEC) +
    'Never mix more than 2 typefaces. Colors: white and warm gold (#F5C66B) accents only.');
  let muestra = null;
  try { muestra = await muestraScriptMarca(tTxt, sec); }
  catch (e) { console.log('[Portada IA] sin muestra tipográfica:', e.message); }
  // PASADA 2 — diseñar la tipografía DENTRO de la ventana del grid.
  const instr = (feedback) =>
    'The FIRST image is a real photograph from an Argentine wood-fired pizzeria: the ' +
    'middle crop of a taller 9:16 Instagram reel cover — exactly the part the profile ' +
    'grid shows.' +
    (muestra ? ' The SECOND image is a TYPOGRAPHY REFERENCE SHEET: the exact texts already ' +
    'rendered in the brand script typeface, black on white. It is ONLY a reference — do ' +
    'not copy it as a white block into the cover.' : '') + '\n' +
    'Add PREMIUM editorial cover typography to the photograph, art-directed and ' +
    'INTEGRATED like a high-end food magazine cover — believable lighting, subtle ' +
    'shadows or soft glow so the type feels designed into the scene, never pasted on ' +
    'top. If any leftover caption, sticker or UI icon remains in the photo, remove it ' +
    'and reconstruct what was behind. Otherwise keep the real scene EXACTLY identical: ' +
    'same dish, hands, lighting, colors, composition and global grading (darken softly ' +
    'ONLY behind the text for legibility, and make that darkening fade out before the ' +
    'top and bottom edges of the canvas — the outermost strips keep the original ' +
    'grading, because this crop is blended back into a taller cover).\n' +
    '- Main title, EXACTLY: "' + tTxt + '"\n' +
    (sec ? '- Secondary line, EXACTLY: "' + sec + '"\n' : '') +
    '- Art direction for the typography: ' + estiloTxt + '\n' +
    (muestra ? '- SCRIPT TYPEFACE: whenever a line is painted in handwritten/script style, ' +
    'COPY the letterforms from the reference sheet EXACTLY — the same thin elegant ' +
    'strokes, curves and long swashes — adapting only size, color, position and ' +
    'lighting. Never invent a different script.\n' : '') +
    '- Perfect kerning, razor-sharp, correctly spelled Spanish with accents.\n' +
    '- LAYOUT: all typography lives in the UPPER HALF of this image, horizontally ' +
    'centered, with generous clear air to the top edge (at least 8% of the height) and ' +
    'to both side edges — no letter or swash ever touches any edge of the image. The ' +
    'dish stays the clearly visible STAR below the text (the text may subtly interact ' +
    'with the top of the scene for depth). NO text in the bottom half.\n' +
    '- Do NOT draw any logo, brand name, watermark or any text beyond the exact lines above.' +
    (feedback ? '\n\nPREVIOUS ATTEMPT HAD ERRORS — fix them now: ' + feedback : '');
  let buf, err = '';
  for (let intento = 0; intento < 3; intento++) {
    const parts = [{ inlineData: { mimeType: 'image/jpeg', data: ventanaBase.toString('base64') } }];
    if (muestra) parts.push({ inlineData: { mimeType: 'image/jpeg', data: muestra.toString('base64') } });
    parts.push({ text: instr(err) });
    const raw = await conReintento(() => llamarGemini(parts, '3:4', PORTADA_MODEL()));
    const ventana = await sharp(raw)
      .resize(1080, PORTADA_VENTANA.height, { fit: 'cover', position: 'centre' })
      .ensureAlpha()
      .composite([{ input: Buffer.from(PORTADA_VENTANA_MASK), blend: 'dest-in' }])
      .png().toBuffer();
    buf = await sharp(limpio)
      .composite([{ input: ventana, top: PORTADA_VENTANA.top, left: 0 }])
      .jpeg({ quality: 92, mozjpeg: true }).toBuffer();
    // Verificación letra por letra (tildes incluidas) + márgenes medidos;
    // un error → hasta 2 reintentos de la pasada de diseño.
    const esperados = 'Título: "' + tTxt + '"' + (sec ? ' · Línea secundaria: "' + sec + '"' : '');
    try {
      const resp = await crearConSchema({
        model: copyModel(),
        max_tokens: 600,
        system: 'Sos control de calidad de portadas. Te paso una portada y los textos que DEBEN aparecer. Compará LETRA POR LETRA (tildes incluidas). Mayúsculas/minúsculas y saltos de línea NO son error. En "errores" poné algo SOLO si (a) un texto esperado falta o está mal escrito, o (b) hay texto, logo o elemento gráfico inventado que no está en la lista (p. ej. un rectángulo o recuadro punteado dibujado). Además estimá SIEMPRE la posición de la tipografía (solo el texto, NO el plato ni la foto): "textoTopPct" = borde superior del texto más alto como % de la altura (0 = borde superior), "textoBottomPct" = borde inferior del texto más bajo, "tocaBordeLateral" = true SOLO si alguna letra literalmente TOCA o se corta en el borde izquierdo o derecho (que quede poco aire NO cuenta como tocar).',
        messages: [{
          role: 'user',
          content: [
            { type: 'image', source: { type: 'base64', media_type: 'image/jpeg', data: buf.toString('base64') } },
            { type: 'text', text: 'Textos esperados → ' + esperados },
          ],
        }],
      }, PORTADA_IA_VERIF_SCHEMA);
      const v = jsonRespuesta(resp) || { ok: true, errores: [] };
      const errs = [...(v.errores || [])];
      // Márgenes por medición del verificador (sobre la portada completa),
      // gate en CÓDIGO. La ventana arranca en 12.5%: texto pegado a su borde
      // superior también se rechaza (queda pegado a la costura).
      if (typeof v.textoTopPct === 'number' && v.textoTopPct < 15) errs.push('el texto empieza demasiado arriba (' + Math.round(v.textoTopPct) + '% de la altura): bajalo, con aire del borde superior');
      if (typeof v.textoBottomPct === 'number' && v.textoBottomPct > 55) errs.push('el bloque de texto termina demasiado abajo (' + Math.round(v.textoBottomPct) + '% de la altura): compactalo en la mitad superior');
      if (v.tocaBordeLateral) errs.push('hay letras tocando los bordes laterales: dejá aire a los costados');
      if (!errs.length) break;
      err = errs.join(' | ');
      console.log('[Portada IA] intento', intento + 1, 'con errores:', err);
    } catch (e) { break; /* verificación caída: entregamos el intento actual */ }
  }
  // Logo real SIEMPRE por código (la IA tiene prohibido dibujarlo). Margen 240
  // (medido de las placas de la marca): queda FUERA del recorte del grid a
  // propósito — dentro chocaría con el contador de vistas que dibuja IG.
  const logo = await prepararLogo('wordmark-blanco', 1080, 1920, 'abajo', 240);
  return logo
    ? sharp(buf).composite(logo.capas).jpeg({ quality: 92, mozjpeg: true }).toBuffer()
    : buf;
}

async function generarPortadaReel({ modo, tema, color, frameBuf, titulo, diseno } = {}) {
  if (!geminiDisponible()) {
  ...
  if (modo === 'limpiar') {
    ...
    // Diseños "IA": el PRO limpia + diseña la tipografía en un solo paso.
    // 'ia' = libre (anclado a marca); 'ia-firma' / 'ia-poster' = estilos de la casa.
    if (titulo && String(diseno || '').startsWith('ia')) {
      return disenoPortadaIA(frameBuf, titulo, PORTADA_IA_BRIEFS[String(diseno).slice(3)]);
    }
    const frame = await sharp(frameBuf).rotate()
  ...
  let base = await sharp(buf).resize(1080, 1920, { fit: 'cover', position: 'centre' })
    .jpeg({ quality: 92, mozjpeg: true }).toBuffer();
  if (modo === 'limpiar') {
    // Con título → tratamiento de marca en el diseño elegido (texto exacto por
    // código + logo real): la captura queda intacta debajo. Sin título → el
    // frame limpio tal cual (ni logo ni scrims).
    return titulo ? overlayTituloCaptura(base, titulo, diseno) : base;
  }
  // Modo generar viejo (hoy el endpoint usa generarPortadaEditorial): header + título simple.
  base = await overlayHeaderPortada(base);
  return overlayTituloPortada(base, titulo);
}

// ============================================================
// PORTADA EDITORIAL "Método Ana" — el modelo PRO (Nano Banana)
// pinta la portada ENTERA con el texto perfecto, guiado por una
// estructura de revista (kicker/hero/titular con 1 palabra
// resaltada/subtítulo). Encima pegamos el logo REAL de PP.
// El copy (título/palabra/subtítulo/hero) lo redacta Claude desde
// un TEMA, con la voz de marca; el usuario lo puede editar y rehacer.
// ============================================================
const PORTADA_MODEL = () => process.env.GEMINI_PORTADA_MODEL || 'gemini-3-pro-image-preview';

const PORTADA_COPY_SCHEMA = {
  type: 'object',
  properties: {
    titulo: { type: 'string' },
    resaltar: { type: 'string' },
    subtitulo: { type: 'string' },
    hero: { type: 'string' },
    color: { type: 'string' },
  },
  required: ['titulo', 'resaltar', 'subtitulo', 'hero', 'color'],
  additionalProperties: false,
};

const PORTADA_COPY_SYSTEM = `Sos el redactor de portadas de reel de Pizzería Popular (pizza al horno de leña, cocina argentina en España). Te paso un TEMA y devolvés un JSON con el copy de UNA portada vertical estilo revista:

{ "titulo": "...", "resaltar": "...", "subtitulo": "...", "hero": "...", "color": "..." }

- "titulo": el gancho, en ESPAÑOL, 3 a 7 palabras, se entiende leído solo, con garra. Se va a mostrar en MAYÚSCULAS. Nada de preguntas vacías ni relleno.
- "resaltar": UNA sola palabra del "titulo" (tiene que aparecer tal cual en él), la más importante, que va dentro de un bloque dorado. Elegí una palabra corta y fuerte; puede quedar al final del bloque.
- "subtitulo": una línea corta en ESPAÑOL que remata o aterriza el título (va en cursiva dorada). Breve.
- "hero": en INGLÉS, la descripción de la ÚNICA imagen protagonista para el modelo de imagen (comida/ambiente de la pizzería: pizza al horno de leña, una porción, cerveza, el fuego, la mesa de madera). Fotográfico, editorial, apetitoso, SIN caras reconocibles, SIN texto.
- "color": en INGLÉS, un hint de paleta (ej "warm amber", "deep red", "golden ember").

Voz de marca: argentina, cálida, directa, sin humo. HONESTIDAD SIEMPRE: la pizzería NO tiene "pantalla gigante" — los partidos se ven por TV; si el tema es fútbol, el hero puede tener una TV al fondo, nunca una pantalla gigante ni un estadio. No prometas nada que el local no tenga (envíos, descuentos, etc.) salvo que el tema lo diga explícito.`;

async function redactarPortadaCopy(tema) {
  const t = String(tema || '').trim();
  if (!t) throw new Error('Contame el tema de la portada.');
  let kitTxt = '';
  try { const { kit } = await getBrandKit(); kitTxt = brandKitTexto(kit); } catch (e) { /* sin kit igual redacta */ }
  const resp = await crearConSchema({
    model: copyModel(),
    max_tokens: 800,
    system: PORTADA_COPY_SYSTEM,
    messages: [{ role: 'user', content: (kitTxt ? kitTxt + '\n\n' : '') + 'TEMA de la portada: ' + t }],
  }, PORTADA_COPY_SCHEMA);
  const out = jsonRespuesta(resp);
  if (!out || !out.titulo) throw new Error('No pude redactar el copy de la portada.');
  return out;
}

// Prompt editorial para el modelo PRO. El TOP queda LIMPIO y vacío: ahí se pega
// el logo real después (por eso: sin kicker, sin nombre de marca, sin logo dibujado).
function promptPortadaEditorial({ titulo, resaltar, subtitulo, hero, color } = {}) {
  const col = String(color || '').trim() || 'warm amber';
  return 'Premium editorial magazine cover, photorealistic, vertical 9:16 (1080x1920), for an ' +
    'Argentine WOOD-FIRED pizzeria. Warm, moody, appetizing: deep espresso/charcoal background ' +
    'with the golden glow of a wood-fired oven and warm ' + col + ' accents (#D8A460). Cinematic ' +
    'dramatic lighting, high-end, uncluttered, generous margins.\n\n' +
    'EXACT EDITORIAL LAYOUT (everything centered):\n' +
    '- TOP 16%: keep it clean and EMPTY — a calm darker band with NO text and no key detail ' +
    '(a real logo will be placed there later).\n' +
    '- UPPER-CENTER: one single strong hero — ' + String(hero || 'a gorgeous wood-fired margherita pizza, editorial food photography, well framed, not cropped') + '.\n' +
    '- LOWER-CENTER: a big bold high-contrast title, WHITE uppercase, that reads EXACTLY "' + titulo + '". ' +
    'Highlight EXACTLY ONE word: the word "' + resaltar + '" sits inside a solid gold (#D8A460) rounded ' +
    'block with dark text. Perfect kerning.\n' +
    (subtitulo ? '- Just under the title: a short subtitle in an elegant warm-gold handwritten script that reads exactly "' + subtitulo + '".\n' : '') +
    '\nRender ALL text razor-sharp, correctly spelled, Spanish accents correct, perfectly legible, ' +
    'premium typography. NO kicker, NO brand name text, NO logo drawn anywhere. No watermarks, no ' +
    'borders, no extra text. Keep the very top band clean and empty.';
}

// Orquesta: (copy desde tema o campos editados) → PRO pinta la portada → logo real arriba.
// Devuelve { buf, copy } para que el panel muestre/edite el texto usado.
async function generarPortadaEditorial({ tema, color, campos } = {}) {
  if (!geminiDisponible()) {
    const e = new Error('La portada para reel necesita la IA de imágenes: falta GEMINI_API_KEY en el .env.');
    e.code = 'NO_KEY';
    throw e;
  }
  const copy = (campos && String(campos.titulo || '').trim())
    ? Object.assign({}, campos)
    : await redactarPortadaCopy(tema);
  if (!copy.resaltar) copy.resaltar = String(copy.titulo || '').split(/\s+/).slice(-1)[0] || '';
  if (!copy.hero) copy.hero = 'a gorgeous wood-fired margherita pizza with melting cheese and leoparded crust, editorial food photography, well framed';
  if (color && String(color).trim()) copy.color = String(color).trim();
  const raw = await llamarGemini([{ text: promptPortadaEditorial(copy) }], '9:16', PORTADA_MODEL());
  let base = await sharp(raw).resize(1080, 1920, { fit: 'cover', position: 'centre' })
    .jpeg({ quality: 92, mozjpeg: true }).toBuffer();
  base = await overlayHeaderPortada(base); // logo real de PP en la franja superior reservada
  return { buf: base, copy };
}
...
4. La composición: formato vertical; el bloque de texto va en los dos tercios inferiores, agrupado y con jerarquía clara; el QUINTO SUPERIOR de la imagen queda TOTALMENTE DESPEJADO — solo foto con aire, sin ningún texto, pill, bandera ni cartel tocando esa franja (ahí se monta el logo real después). Margen respirado en los bordes.
5. Si hay CTA: es un botón pill dorado con el texto en oscuro. Si hay ubicación: etiqueta chica en mayúsculas espaciadas. Si hay cartel de evento: pill dorado chico en mayúsculas, PEGADO al bloque de texto — va inmediatamente arriba del título, como primera línea del bloque (en la mitad inferior de la imagen, nunca flotando solo cerca del borde superior). Si hay banderas: una fila compacta con la bandera REAL de cada país y su NOMBRE escrito al lado en mayúsculas chicas (bandera + "FRANCIA", un "VS" dorado en el medio, bandera + "ESPAÑA" — con los nombres reales que te pasen), TAMBIÉN pegada al bloque de texto, entre el pill de evento y el título. El orden vertical del bloque es: pill de evento → fila de banderas → título → remate → bajada → botón, todo junto en la mitad inferior.
...
- El remate caligráfico va en su PROPIA línea, entre o debajo de las líneas del título, con aire: puede rozar elegantemente, pero PROHIBIDO montarlo encima de otro texto — cada texto tiene que leerse completo de un vistazo.
...
async function redactarPromptPlaca({ placa, formato, feedback } = {}) {
  ...
  const banderas = (Array.isArray(placa.banderas) && placa.banderas.length >= 2)
    ? 'Fila de banderas CON NOMBRES (va por debajo de la franja superior despejada, nunca a la altura del logo): bandera de ' + nombrePais(placa.banderas[0]) + ' + texto "' + nombrePais(placa.banderas[0]).toUpperCase() + '", "VS" dorado, bandera de ' + nombrePais(placa.banderas[1]) + ' + texto "' + nombrePais(placa.banderas[1]).toUpperCase() + '". Los nombres de los países son textos EXACTOS de la placa.'
    : '';
  const partes = [
    brandKitTexto(kit),
...
- "errores" (GRAVES, obligan a regenerar): un texto esperado que falta o está MAL ESCRITO (faltas de ortografía, tildes mal, letras deformes o ilegibles, palabras cambiadas); un texto esperado que aparece DUPLICADO (la misma frase dos veces en la placa); texto INVENTADO que no está en la lista (palabras, números, precios, carteles, códigos de color, inscripciones en la ropa/camisetas — lo que sea legible); texto, pills o carteles en la FRANJA SUPERIOR CENTRAL — el quinto de arriba de la imagen — (esa zona debe quedar libre: ahí se monta el logo real después; un pill de evento tipo "EN VIVO" metido ahí TAMBIÉN es error: reportalo como "el cartel de evento invade la franja del logo: bajalo"; una fila de BANDERAS metida ahí o flanqueando el centro de esa franja TAMBIÉN es error: reportalo como "las banderas invaden la franja del logo: bajalas debajo del cartel de evento"); un logo, isotipo o letra P dibujados (el logo real se agrega después: NO debe haber ninguno); una bandera de país incorrecta si se esperaban banderas; dos textos MONTADOS uno sobre otro al punto de que alguno no se lea completo de un vistazo (típico: la caligrafía dorada impresa ENCIMA de una línea del título — reportalo como "el remate caligráfico está montado sobre el título y no se lee: separalos en líneas distintas"). Entrelazado elegante con aire está bien; superposición que ensucia la lectura es error grave.
- "errores" TAMBIÉN incluye romper la ESTÉTICA DE MARCA: la marca es CÁLIDA (ámbar, dorado, rojo profundo, negro, madera) con caligrafía del remate SIEMPRE dorada. Es error grave si la placa está dominada por colores fríos o ajenos (azul, verde, violeta, rosa chicle), o si el remate caligráfico salió de otro color que no sea dorado/crema.
- "avisos" (menores, NO obligan a regenerar): contraste flojo, texto muy pegado al borde, jerarquía confusa, tipografía que no respeta el estilo pedido, clima de marca correcto pero flojo.
- "leido": TODO el texto que ves en la placa, transcripto tal cual.
- PROCEDIMIENTO OBLIGATORIO antes de responder: recorré la placa de arriba a abajo y CONTÁ cuántas veces aparece CADA texto esperado (incluido el cartel de evento). Si alguno aparece 2 o más veces — aunque sea en zonas distintas, con otra caja o tamaño — es error grave: reportalo como 'el texto "..." está duplicado: dejá uno solo'.
...
async function verificarPlacaIA(buf, placa) {
  ...
  const banderas = (Array.isArray(placa.banderas) && placa.banderas.length >= 2)
    ? 'Deben verse las banderas de ' + nombrePais(placa.banderas[0]) + ' y ' + nombrePais(placa.banderas[1]) + ' (en ese orden, con un VS) CON sus nombres escritos al lado: "' + nombrePais(placa.banderas[0]).toUpperCase() + '" y "' + nombrePais(placa.banderas[1]).toUpperCase() + '" son textos esperados (si faltan, es error). La fila de banderas NO puede estar en la franja superior del logo.'
...
async function generarPlacaCompletaIA(placa, formato) {
  ...
  // Referencias REALES opcionales (si el copy las eligió): foto del local
  // (ambienteReal) y/o foto del producto (productoReal). Si no bajan, se genera igual.
  const bajarRef = async (url) => {
    try {
  ...
  const ambParte = placa.fotoUrl ? await bajarRef(placa.fotoUrl) : null;
  const prodParte = placa.fotoProductoUrl ? await bajarRef(placa.fotoProductoUrl) : null;
  let feedback = '';
  ...
  for (let intento = 1; intento <= 2; intento++) {
    ...
    const notas = [];
    const ORD = ['primera', 'segunda', 'tercera', 'cuarta'];
    let nRef = 0;
    if (refParts.length) {
      notas.push((refParts.length === 1 ? 'La primera imagen adjunta es una placa REAL' : 'Las primeras ' + refParts.length + ' imágenes adjuntas son placas REALES') + ' de la marca: usala' + (refParts.length === 1 ? '' : 's') + ' como referencia de estilo gráfico (paleta, clima, jerarquía). NO copies sus textos, ni sus carteles/pills, ni reproduzcas su logo.');
      nRef += refParts.length;
    }
    if (ambParte) {
      notas.push('La ' + (ORD[nRef] || 'siguiente') + ' imagen adjunta es una foto REAL de nuestro local: usala como referencia de la AMBIENTACIÓN de la escena (arquitectura, materiales, mobiliario, iluminación). No copies su encuadre literal, y NO reproduzcas los letreros, carteles ni textos que aparezcan en esa foto: ningún texto de la foto va en la placa.');
      nRef += 1;
    }
    if (prodParte) {
      notas.push('La ' + (ORD[nRef] || 'siguiente') + ' imagen adjunta es una foto REAL de NUESTRO PRODUCTO: el plato de la placa debe verse como ESE (mismo tipo de plato, misma masa/preparación, mismos toppings y punto de cocción — no comida genérica de stock). Podés cambiar encuadre, fondo e iluminación para la escena, pero el producto es ese. No copies el fondo de esa foto ni ningún texto que aparezca en ella.');
      nRef += 1;
    }
    const parts = [...refParts, ...(ambParte ? [ambParte] : []), ...(prodParte ? [prodParte] : []),
      { text: (notas.length ? notas.join(' ') + '\n\n' : '') + prompt }];
    // Modelo PRO también acá (13 jul): el flash pintaba letras con duplicados y
    // superposiciones; el PRO es el mismo que borda la tipografía en portadas.
    // Override por env GEMINI_PLACA_MODEL si hay que volver atrás sin deploy.
    const buf = await llamarGemini(parts, ASPECTO_IA[formato] || '4:5',
      process.env.GEMINI_PLACA_MODEL || PORTADA_MODEL());
    let verif;
    try {
...
// EDICIÓN QUIRÚRGICA (13 jul): un ajuste chico EDITA la placa que ya existe en
// vez de regenerarla — misma escena, mismo encuadre, solo el cambio pedido.
// Se dispara cuando la placa trae _edicionIA (la instrucción cruda del usuario)
// y hay imagen cacheada. Regenerar de cero queda solo para cambios de foto/escena.
async function editarPlacaIA(baseBuf, placa, formato) {
  const avisos = [];
  const basePart = {
    inlineData: {
      mimeType: 'image/jpeg',
      data: (await sharp(baseBuf).jpeg({ quality: 92 }).toBuffer()).toString('base64'),
    },
  };
  let mejor = null;
  let feedback = '';
  for (let intento = 1; intento <= 2; intento++) {
    const instr = 'Editá la imagen adjunta (una placa de redes ya diseñada). Aplicá SOLO este cambio pedido por el usuario: "' +
      String(placa._edicionIA).trim() + '". TODO lo demás queda EXACTAMENTE igual: misma escena de fondo, mismo encuadre, ' +
      'mismos colores y tipografías, y los demás textos quedan donde están, letra por letra. Si el cambio pide sacar un texto ' +
      'o cartel, rellená esa zona con el fondo de la escena, sin dejar marca. Si pide reemplazar un texto, escribí el nuevo ' +
      'EXACTO, con sus tildes y eñes, en el mismo estilo del que reemplaza. No agregues elementos, textos ni logos nuevos.' +
      (feedback ? ' ERRORES DEL INTENTO ANTERIOR (corregilos sí o sí): ' + feedback : '');
    const buf = await llamarGemini([basePart, { text: instr }],
      (formato === 'historia' ? '9:16' : '4:5'),
      process.env.GEMINI_PLACA_MODEL || PORTADA_MODEL());
    let verif;
    try {
      verif = await verificarPlacaIA(buf, placa);
    } catch (e) {
      avisos.push('No pude verificar los textos de la placa editada (' + e.message + '): revisala a ojo.');
      return { buf, avisos };
    }
    if (!mejor || verif.errores.length < mejor.verif.errores.length) mejor = { buf, verif };
    if (!verif.errores.length) {
      verif.avisos.forEach((a) => avisos.push(a));
      return { buf, avisos };
    }
    feedback = verif.errores.join(' · ');
    console.error('[Gen placa IA · edición] intento ' + intento + ' con errores:', feedback);
  }
  mejor.verif.errores.forEach((e) => avisos.push('⚠️ La edición no quedó perfecta: ' + e + ' — tocá «Aplicar y rehacer» de nuevo para otro intento.'));
  mejor.verif.avisos.forEach((a) => avisos.push(a));
  return { buf: mejor.buf, avisos, fallida: true };
}
...
function firmaPlacaIA(placa) {
  return JSON.stringify([
    ...
    placa.fotoProductoUrl || '', // referencia de producto real: ídem
  ]);
}
...
async function prepararLogo(variante, W, H, pos, margen) {
  ...
  const margin = margen != null ? margen : (esHistoria ? 86 : 64);
...
async function generarPiezas(formato, placas) {
  ...
  for (let i = 0; i < placas.length; i++) {
    ...
    if (p.modoIA === 'completa') {
      ...
      // Edición quirúrgica: si el ajuste dejó _edicionIA y hay imagen previa, se
      // baja la placa existente aunque la firma haya cambiado (los textos nuevos
      // los aplica la edición, no una regeneración de cero).
      const editar = p._edicionIA && p.iaPlacaUrl;
      if (p.iaPlacaUrl && (p.iaPlacaFirma === firma || editar)) {
        try {
      ...
      if (crudo && editar) {
        const gen = await editarPlacaIA(crudo, p, f);
        crudo = gen.buf;
        gen.avisos.forEach((a) => avisos.push('Placa ' + (i + 1) + ': ' + a));
        p.avisosIA = gen.avisos;
        if (gen.fallida) {
          // La edición no quedó perfecta: se entrega igual (con avisos) pero no se
          // pisa el cache — reintentar edita de nuevo sobre la versión anterior sana.
        } else {
          try {
      ...
      delete p._edicionIA; // consumida (o regenerada de cero): no arrastrarla al próximo componer
...
module.exports = { generarCopy, ajustarCopy, generarPiezas, generarImagenIA, generarPortadaReel, generarPortadaEditorial, disenoPortadaIA, overlayTituloPortada, overlayTituloCaptura, afinarPromptIA, sugerirEscenaBlog, geminiDisponible, materializarFoto, componerPlaca, fotoDePlaca, interpretarRetoque, redactarPromptPlaca, verificarPlacaIA, generarPlacaCompletaIA, terminarPlacaIA, sanearCopy, acortarLargos, FORMATOS, LOGOS, ESTILO_LIST };
