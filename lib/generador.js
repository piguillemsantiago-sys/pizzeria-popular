...
function svgTexto(placa, W, H, numero, total, zona) {
  ...
  // Acento (pincel): se achica para entrar en UNA línea y que el bloque no se
  // desborde sobre las caras. Grande, pero ajustado al ancho.
  const acentoTxt = String(placa.acento || '').trim();
  let aSize = Math.round((esHistoria ? 158 : 132) * est.aFactor);
  if (acentoTxt) {
  ...
  // Posición vertical del bloque. Arriba arranca alto (debajo del logo) para
  // dejar libre el centro/abajo donde suelen estar las caras.
  let y;
  if (vert === 'arriba') y = esHistoria ? 210 : 170;
  else if (vert === 'centro') y = Math.max(esHistoria ? 300 : 230, (H - bloqueH) / 2);
...
Devolvé SOLO un JSON: { "vert": "arriba"|"abajo"|"centro", "horiz": "centro" }

Criterio de diseñador (lo más importante: NO TAPAR CARAS):
- El bloque de texto es ALTO (ocupa como un tercio de la imagen). Elegí "arriba" o
  "abajo" según dónde haya MÁS espacio libre y tranquilo, lejos de las caras.
- Mirá a qué ALTURA están las caras. Si las caras están en el centro/abajo → "arriba".
  Si las caras están arriba → "abajo". NUNCA pongas el texto encima de las caras.
- "centro" solo si el centro está despejado (sin caras ni el plato principal).
- horiz siempre "centro".`;
