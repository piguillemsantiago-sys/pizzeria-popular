...

{
  ...
  "placas": [
    { "titulo": "...", "acento": "...", "bajada": "...", "cta": "...", "lugar": "...",
      "estilo": "clasico|pincelxl|reglas|invertido",
      "logo": "iso-blanco|iso-fuego|iso-rojo|iso-verde|wordmark-blanco|wordmark-oscuro",
      "cambiarFoto": false, "fotoHint": "" }
  ]
}

En la gráfica, "titulo" va en MAYÚSCULAS blancas y "acento" va grande en pincel dorado.
"lugar" es un tag de ubicación (ej: "Valencia", "Av. Niza 9, Alicante") o "".
"estilo" es el DISEÑO de la pieza: clasico (título arriba, pincel debajo), pincelxl
(pincel gigante protagonista), reglas (pincel arriba, título con líneas doradas) o
invertido (pincel arriba, título debajo). Cambialo si piden otro diseño/variar; si no
lo mencionan, dejá el actual.
...
// 4 diseños de composición (todos comparten el ADN de marca: logo P, pincel
// dorado, blanco, centrado). El generador ROTA entre ellos para variar; la
// posición vertical la decide el criterio (ubicarTexto). 'orden' define cómo
// se apilan los elementos; aFactor el tamaño del pincel; reglas las líneas
// doradas que flanquean el título.
const DISENOS = {
  clasico:   { orden: ['tag', 'titulo', 'acento', 'bajada', 'cta'], aFactor: 1.0,  reglas: false },
  pincelxl:  { orden: ['tag', 'titulo', 'acento', 'bajada', 'cta'], aFactor: 1.5,  reglas: false, tituloChico: true },
  reglas:    { orden: ['tag', 'acento', 'titulo', 'bajada', 'cta'], aFactor: 1.0,  reglas: true },
  invertido: { orden: ['tag', 'acento', 'titulo', 'bajada', 'cta'], aFactor: 1.15, reglas: false },
};
const DISENO_LIST = ['clasico', 'pincelxl', 'reglas', 'invertido'];
...
// SVG del texto de la placa, con uno de los 4 DISEÑOS (composición). Todo
// centrado, título MAYÚSCULAS, pincel dorado (Kaushan). La posición vertical
// la dicta `zona` (criterio según la foto: dónde no tapar caras/comida).
function svgTexto(placa, W, H, numero, total, zona) {
  ...
  const lugSize = 27;
  const tSize = dis.tituloChico ? (esHistoria ? 34 : 30) : (esHistoria ? 56 : 50);
  const bSize = esHistoria ? 36 : 33;

  // Acento (pincel) ajustado al ancho para entrar en una línea, grande.
  const acentoTxt = String(placa.acento || '').trim();
  let aSize = Math.round((esHistoria ? 152 : 128) * dis.aFactor);
  if (acentoTxt) {
    const fit = Math.floor(ancho / (acentoTxt.length * 0.46));
    aSize = Math.min(aSize, Math.max(esHistoria ? 78 : 66, fit));
  }

  const lugar = String(placa.lugar || '').trim();
  const tLines = wrap(placa.titulo, esHistoria ? 22 : 24);
  const aLines = wrap(acentoTxt, Math.max(6, Math.round(ancho / (aSize * 0.46))));
  const bLines = wrap(placa.bajada, esHistoria ? 40 : 42);

  const tLH = Math.round(tSize * 1.18);
  const aLH = Math.round(aSize * 0.92);
  const bLH = Math.round(bSize * 1.4);
  const lugLH = 42;
  const ctaH = placa.cta ? 104 : 0;

  // Cada fila: { h, gap (después), draw(yTop) → svg }. yTop = tope de la fila.
  const fila = {
    tag: () => lugar ? { h: lugLH, gap: 16, draw(yTop) {
      const y = yTop + lugSize;
      const txt = '<tspan fill="' + GOLD + '">•</tspan>  ' + escXml(lugar.toUpperCase());
      const a = 'font-family="Montserrat" font-weight="bold" font-size="' + lugSize + '" letter-spacing="2" text-anchor="middle"';
      return '<text x="' + cx + '" y="' + (y + 2) + '" ' + a + ' fill="#000" opacity=".45">' + txt + '</text>' +
        '<text x="' + cx + '" y="' + y + '" ' + a + ' fill="#fff">' + txt + '</text>';
    } } : null,
    titulo: () => tLines.length ? { h: tLines.length * tLH, gap: 18, draw(yTop) {
      let y = yTop, s = '';
      for (const line of tLines) {
        y += tLH;
        const a = 'font-family="Montserrat" font-weight="bold" font-size="' + tSize + '" letter-spacing="3" text-anchor="middle"';
        s += txtSombra(cx, y, a, escXml(line.toUpperCase()), '#fff');
      }
      if (dis.reglas) { // líneas doradas flanqueando la última línea
        const last = tLines[tLines.length - 1].toUpperCase();
        const tw = last.length * tSize * 0.62;
        const cyl = y - tSize * 0.34;
        const gap = 30, lineLen = (W / 2) - (tw / 2) - gap - margenL;
        if (lineLen > 34) {
          s += '<rect x="' + margenL + '" y="' + (cyl - 2) + '" width="' + lineLen + '" height="3" fill="' + GOLD + '"/>';
          s += '<rect x="' + (W - margenL - lineLen) + '" y="' + (cyl - 2) + '" width="' + lineLen + '" height="3" fill="' + GOLD + '"/>';
        }
      }
      return s;
    } } : null,
    acento: () => aLines.length ? { h: aLines.length * aLH, gap: 16, draw(yTop) {
      let y = yTop, s = '';
      for (const line of aLines) {
        y += aLH;
        const a = 'font-family="Kaushan Script" font-size="' + aSize + '" text-anchor="middle"';
        s += txtSombra(cx, y, a, escXml(line), GOLD);
      }
      return s;
    } } : null,
    bajada: () => bLines.length ? { h: bLines.length * bLH, gap: 22, draw(yTop) {
      let y = yTop, s = '';
      for (const line of bLines) {
        y += bLH;
        const a = 'font-family="Montserrat" font-size="' + bSize + '" text-anchor="middle"';
        s += txtSombra(cx, y, a, escXml(line), '#f2efe9');
      }
      return s;
    } } : null,
    cta: () => placa.cta ? { h: ctaH, gap: 0, draw(yTop) {
      const t = escXml(placa.cta);
      const cw = Math.max(300, t.length * 24 + 90);
      const cxx = (W - cw) / 2;
      return '<rect x="' + cxx + '" y="' + yTop + '" rx="52" width="' + cw + '" height="' + ctaH + '" fill="' + GOLD + '"/>' +
        '<text x="' + (cxx + cw / 2) + '" y="' + (yTop + ctaH / 2 + 13) + '" text-anchor="middle" font-family="Montserrat" font-weight="bold" font-size="38" fill="' + DARK + '">' + t + '</text>';
    } } : null,
  };

  const filas = dis.orden.map((k) => fila[k]()).filter(Boolean);
  let bloqueH = 0;
  filas.forEach((f, i) => { bloqueH += f.h + (i < filas.length - 1 ? f.gap : 0); });

  let y0;
  if (vert === 'arriba') y0 = esHistoria ? 210 : 170;
  else if (vert === 'centro') y0 = Math.max(esHistoria ? 300 : 230, (H - bloqueH) / 2);
  else y0 = H - margenB - bloqueH;
  ...
  let yc = y0;
  filas.forEach((f, i) => { s += f.draw(yc); yc += f.h + (i < filas.length - 1 ? f.gap : 0); });

  // Scrim detrás del bloque, para despegarlo del fondo.
  const pad = 56;
  const scrimTop = Math.max(0, y0 - pad), scrimBot = Math.min(H, yc + pad);
  ...
  const scrimRect = '<rect x="0" y="' + scrimTop + '" width="' + W + '" height="' + (scrimBot - scrimTop) + '" fill="url(#scrim)"/>';
...
async function componerPlaca(placa, fotoBuf, formato, numero, total) {
  ...
  const foto = await sharp(fotoBuf).rotate()
    ...
    .toBuffer();
  // La posición vertical del texto la decide el criterio (franja más vacía),
  // independiente del diseño elegido.
  const zona = await ubicarTexto(foto);
  const capas = [{ input: svgTexto(placa, w, h, numero, total, zona), left: 0, top: 0 }];
  // El isologo va SIEMPRE (la marca tiene que estar en todas las piezas).
