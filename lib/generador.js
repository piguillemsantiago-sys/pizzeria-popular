...
async function generarCopy(instruccion, formato) {
  ...
  // Asigna un estilo rotando desde un punto al azar, para que no salgan
  // todas iguales (y que un carrusel varíe placa a placa).
  const off = Math.floor(Math.random() * ESTILO_LIST.length);
  out.placas.forEach((p, i) => { p.estilo = ESTILO_LIST[(off + i) % ESTILO_LIST.length]; });
  return out;
}
...

{
  ...
  "placas": [
    { "titulo": "...", "acento": "...", "bajada": "...", "cta": "...", "lugar": "...",
      "estilo": "editorial|hero|centrado|alto",
...
"estilo" es el layout: editorial (texto abajo), hero (acento más grande), centrado
(todo al centro) o alto (texto arriba). Cambialo si piden mover/centrar el texto;
si no lo mencionan, dejá el actual.
...
// dorada (Great Vibes) + bajada + CTA, con el layout que dicte el estilo.
function svgTexto(placa, W, H, numero, total) {
  const esHistoria = H > 1500;
  const est = ESTILOS[placa.estilo] || ESTILOS.editorial;
  const centrado = est.align === 'center';

  const tSize = esHistoria ? 58 : 52;
  const aSize = Math.round((esHistoria ? 140 : 118) * est.aFactor);
  ...
  // x y anchor según alineación.
  const tx = centrado ? W / 2 : margenL;
  const anchor = centrado ? ' text-anchor="middle"' : '';
  ...
  // Posición vertical del bloque según estilo.
  let y;
  if (est.pos === 'top') y = esHistoria ? 300 : 250;
  else if (est.pos === 'center') y = Math.max(esHistoria ? 320 : 240, (H - bloqueH) / 2);
  ...
  if (lugar) {
    ...
    s += '<text x="' + tx + '" y="' + y + '"' + anchor + ' font-family="Montserrat" font-weight="bold" ' +
      'font-size="' + lugSize + '" letter-spacing="2" fill="#fff" opacity=".92">' + txt + '</text>';
    y += 14;
  }
  ...
  if (aLines.length) {
    ...
    for (const line of aLines) {
      y += aLH;
      s += '<text x="' + (centrado ? W / 2 : margenL - 4) + '" y="' + y + '"' + anchor +
        ' font-family="Great Vibes" font-size="' + aSize + '" fill="' + GOLD + '">' + escXml(line) + '</text>';
    }
  }
  ...
  if (bLines.length) {
    ...
    for (const line of bLines) {
      y += bLH;
      s += '<text x="' + tx + '" y="' + y + '"' + anchor + ' font-family="Montserrat" font-size="' + bSize +
  ...
  if (placa.cta) {
    ...
    const ctaX = centrado ? (W - ctaW) / 2 : margenL;
  ...
  return Buffer.from(
    ...
    '<defs>' + gradiente(est.trato, W, H) + '</defs>' +
