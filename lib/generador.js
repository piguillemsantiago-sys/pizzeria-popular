...
const PORTADA_IA_BRIEFS = {
  firma:
    'The main title as a HUGE gold (#F5C66B) handwritten script, like a chef signature ' +
    'crossing the frame — COPY the exact letterforms of the brand script shown in the ' +
    'typography reference sheet. The secondary line below in clean white geometric sans ' +
    '(style of "Montserrat"), uppercase, generously letterspaced, LARGE enough to read ' +
    'easily on a phone. ' + PORTADA_IA_LEGIBILIDAD,
  poster:
...
async function disenoPortadaIA(frameBuf, titulo, estilo) {
  ...
  const instr = (feedback) =>
    'The FIRST image is a real photograph (a vertical 9:16 reel frame) from an Argentine ' +
    'wood-fired pizzeria — the one you must edit.' +
    ...
    'STEP 1 — CLEAN: remove any overlaid captions, stickers, emojis or UI icons and ' +
    'reconstruct what was behind them. Keep the real scene EXACTLY identical: same dish, ' +
    'hands, lighting, colors and composition. Do NOT restyle the photo.\n' +
    'STEP 2 — DESIGN: add PREMIUM editorial cover typography, art-directed and INTEGRATED ' +
    'into the photograph like a high-end food magazine cover — believable lighting, subtle ' +
    'shadows or soft glow so the type feels designed into the scene, never pasted on top:\n' +
    ...
    '- Composition: place the text where it looks BEST for this photo (it may subtly ' +
    'interact with the scene for depth). Generous margins. Keep the BOTTOM 300px and the ' +
    'TOP 160px completely free of text (a real logo goes at the bottom later).\n' +
  ...
  for (let intento = 0; intento < 2; intento++) {
    const parts = [{ inlineData: { mimeType: 'image/jpeg', data: frame.toString('base64') } }];
    if (muestra) parts.push({ inlineData: { mimeType: 'image/jpeg', data: muestra.toString('base64') } });
    parts.push({ text: instr(err) });
    const raw = await llamarGemini(parts, '9:16', PORTADA_MODEL());
    buf = await sharp(raw).resize(1080, 1920, { fit: 'cover', position: 'centre' })
      .jpeg({ quality: 92, mozjpeg: true }).toBuffer();
    // Verificación letra por letra (tildes incluidas); un error → 1 reintento.
    const esperados = 'Título: "' + tTxt + '"' + (sec ? ' · Línea secundaria: "' + sec + '"' : '');
    try {
      const resp = await crearConSchema({
        model: copyModel(),
        max_tokens: 600,
        system: 'Sos control de calidad de portadas. Te paso una portada y los textos que DEBEN aparecer. Compará LETRA POR LETRA (tildes incluidas). Mayúsculas/minúsculas y saltos de línea NO son error. Devolvé JSON {"ok":bool,"errores":[...],"leido":"..."}: en "errores" SOLO si un texto esperado falta o está mal escrito, o si hay texto/logo inventado que no está en la lista.',
        messages: [{
          role: 'user',
          content: [
