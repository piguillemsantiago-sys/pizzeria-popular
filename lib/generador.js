...
- "escenaIA": un PROMPT (en español) para generar la FOTO DE FONDO con IA (Gemini),
  ...
  · Excepción: si la instrucción pide gente/hinchas/ambiente humano, incluila pero
    ...
    mirando la pantalla"), NUNCA caras nítidas ni en primer plano.
...
async function generarPlacaCompletaIA(placa, formato) {
  ...
  for (let intento = 1; intento <= 2; intento++) {
    ...
    const parts = refParts.length
      ? [...refParts, { text: 'Las imágenes adjuntas son placas REALES de la marca: usalas como referencia de estilo gráfico (paleta, clima, jerarquía). NO copies sus textos, ni sus carteles/pills, ni reproduzcas su logo.\n\n' + prompt }]
      : [{ text: prompt }];
  ...
  for (let i = 0; i < placas.length; i++) {
    ...
    if (p.modoIA === 'completa') {
      ...
      if (!crudo) {
        ...
        // Los avisos viajan EN la placa: el panel los muestra pegados a la pieza y
        // persisten mientras la imagen cacheada (posiblemente fallada) siga en uso.
