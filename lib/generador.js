...
async function generarPortadaReel({ modo, tema, color, frameBuf, titulo } = {}) {
  ...
  let base = await sharp(buf).resize(1080, 1920, { fit: 'cover', position: 'centre' })
    .jpeg({ quality: 92, mozjpeg: true }).toBuffer();
  // Header de marca SOLO en modo generar: el prompt le reserva el top 15% oscuro.
  // En 'limpiar' respetamos el frame real intacto (como el flan ya aprobado).
  if (modo !== 'limpiar') base = await overlayHeaderPortada(base);
  // Título opcional, dibujado con texto exacto encima (nunca por la IA).
...
module.exports = { generarCopy, ajustarCopy, generarPiezas, generarImagenIA, generarPortadaReel, generarPortadaEditorial, overlayTituloPortada, afinarPromptIA, sugerirEscenaBlog, geminiDisponible, materializarFoto, componerPlaca, fotoDePlaca, interpretarRetoque, redactarPromptPlaca, verificarPlacaIA, generarPlacaCompletaIA, terminarPlacaIA, sanearCopy, acortarLargos, FORMATOS, LOGOS, ESTILO_LIST };
