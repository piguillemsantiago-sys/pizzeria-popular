...
async function generarImagenIA(prompt, opts) {
  ...
  const res = await fetch(
    'https://generativelanguage.googleapis.com/v1beta/models/' + model + ':generateContent',
    {
      ...
      // imageConfig: SOLO aspectRatio. ⚠️ NO agregar imageSize ('2K' u otro): probado en
      // el VPS (flash y pro), imageSize degrada la generación a un BORRÓN ámbar vacío sin
      // sujeto. Sin él, sale nítida. La placa se recorta a 1080×1920 con sharp igual.
      body: JSON.stringify({
        contents: [{ parts: reqParts }],
        generationConfig: { imageConfig: { aspectRatio: opts.aspecto || '9:16' } },
      }),
    }
  );
...
const DISENOS = {
  clasico:   { orden: ['tag', 'titulo', 'acento', 'bajada', 'cta'], font: 'sans',  align: 'left',   anchor: 'auto',   aFactor: 1.0 },
  ...
  producto:  { orden: ['titulo', 'acento', 'cta'],                  font: 'serif', align: 'left',   anchor: 'abajo',  titGrande: true, acentoHero: true, gapFactor: 0.32 },
};
