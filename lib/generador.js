...
// Máscara de fundido (90px arriba/abajo) para reintegrar la ventana diseñada
// a la portada completa sin costura visible.
const PORTADA_VENTANA_MASK =
  '<svg xmlns="http://www.w3.org/2000/svg" width="1080" height="1440"><defs>' +
  '<linearGradient id="f" x1="0" y1="0" x2="0" y2="1">' +
  '<stop offset="0" stop-color="#fff" stop-opacity="0"/>' +
  '<stop offset="0.0625" stop-color="#fff" stop-opacity="1"/>' +
...
async function disenoPortadaIA(frameBuf, titulo, estilo) {
  ...
  const limpioRaw = await llamarGemini([
  ...
  const instr = (feedback) =>
    ...
    'ONLY behind the text for legibility).\n' +
  ...
  for (let intento = 0; intento < 3; intento++) {
    ...
    const raw = await llamarGemini(parts, '3:4', PORTADA_MODEL());
