...
// Título EDITORIAL sobre una captura limpia, con el lenguaje de las PLACAS DE
// REFERENCIA reales de la marca (ver refs Drive, ej. "Milanesa Buenos Aires /
// fugazzeta"): título SERIF blanco de alto contraste (Abril Fatface) + remate
// en CURSIVA dorada (Abuget) + wordmark real centrado ABAJO. Sin chips ni
// bloques. Todo por CÓDIGO con tinta real medida (trim) y sombra suave: texto
// exacto, imagen intacta. El campo admite "Título | remate en cursiva".
async function overlayTituloEditorial(imgBuf, titulo) {
  const raw = String(titulo || '').trim();
  if (!raw) return imgBuf;
  const [tTxt, remate] = raw.split('|').map((s) => String(s || '').trim());
  const W = 1080, H = 1920;
  ...
  // Renderiza un texto, lo recorta a la tinta real (trim) y le arma una sombra
  // suave (silueta negra difuminada, como el logo) para que lea sobre foto clara.
  const pieza = async (txt, fill, font, fsz) => {
    const wEst = Math.ceil(txt.length * fsz * 1.2) + 160;
    const hEst = Math.ceil(fsz * 2.1);
    const svg = '<svg xmlns="http://www.w3.org/2000/svg" width="' + wEst + '" height="' + hEst + '">' +
      '<text x="14" y="' + Math.round(fsz * 1.3) + '" font-family="' + font + '" ' +
  ...
  if (remate) filas.push(await pieza(remate, GOLD, BRUSH, Math.round(fs * 1.15)));

  // Bloque centrado sobre el tercio inferior; debajo queda el wordmark real.
  const bottom = 1690;
  const capas = [];
  for (let i = 0; i < filas.length; i++) {
    const p = filas[i];
    const esRemate = remate && i === filas.length - 1;
    const rowBottom = bottom - (filas.length - 1 - i) * lh + (esRemate ? Math.round(fs * 0.18) : 0);
    const left = Math.round((W - p.w) / 2);
    const top = rowBottom - p.h;
    capas.push({ input: p.sombra, left: left - 14, top: top - 14 + 3 });
    capas.push({ input: p.buf, left, top });
  }

  // Scrim inferior: sostiene título + logo sobre cualquier foto (refs: la foto
  // se apaga hacia abajo y el bloque de texto vive en esa penumbra).
  const blockTop = bottom - filas.length * lh - 90;
  const scrim = '<svg xmlns="http://www.w3.org/2000/svg" width="' + W + '" height="' + H + '">' +
    '<defs><linearGradient id="b" x1="0" y1="0" x2="0" y2="1">' +
    '<stop offset="0" stop-color="#000" stop-opacity="0"/>' +
    '<stop offset=".45" stop-color="#000" stop-opacity=".5"/>' +
    '<stop offset="1" stop-color="#000" stop-opacity=".85"/></linearGradient></defs>' +
    '<rect x="0" y="' + blockTop + '" width="' + W + '" height="' + (H - blockTop) + '" fill="url(#b)"/></svg>';
  ...
  const out = logo
    ? await sharp(conTexto).composite(logo.capas).jpeg({ quality: 92, mozjpeg: true }).toBuffer()
    : await sharp(conTexto).jpeg({ quality: 92, mozjpeg: true }).toBuffer();
  return out;
}

async function generarPortadaReel({ modo, tema, color, frameBuf, titulo } = {}) {
  if (!geminiDisponible()) {
  ...
  let base = await sharp(buf).resize(1080, 1920, { fit: 'cover', position: 'centre' })
    .jpeg({ quality: 92, mozjpeg: true }).toBuffer();
  if (modo === 'limpiar') {
    // Con título → tratamiento EDITORIAL (título grande + palabra en bloque
    // dorado + logo real), todo por código: la captura queda intacta debajo.
    // Sin título → el frame limpio tal cual (ni logo ni scrims).
    return titulo ? overlayTituloEditorial(base, titulo) : base;
  }
...
module.exports = { generarCopy, ajustarCopy, generarPiezas, generarImagenIA, generarPortadaReel, generarPortadaEditorial, overlayTituloPortada, overlayTituloEditorial, afinarPromptIA, sugerirEscenaBlog, geminiDisponible, materializarFoto, componerPlaca, fotoDePlaca, interpretarRetoque, redactarPromptPlaca, verificarPlacaIA, generarPlacaCompletaIA, terminarPlacaIA, sanearCopy, acortarLargos, FORMATOS, LOGOS, ESTILO_LIST };
