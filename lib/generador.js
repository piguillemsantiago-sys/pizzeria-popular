...
const { referenciaVisual } = require('./banco');
...
async function generarImagenIA(prompt) {
  ...
  // Referencia de marca: guía de estilo (texto, desde las placas) + imágenes de
  // referencia visual. Para las IMÁGENES preferimos FOTOS REALES del banco (sin
  // texto horneado, fieles al producto); si el banco no está indexado, caemos a
  // las placas de referencia. Sólo paleta/luz/mood, nunca copiar texto/logos.
  const reqParts = [];
  ...
  let refs = [];
  try { refs = await referenciaVisual(3); } catch (e) { refs = []; }
  if (!refs.length && referenciaActiva()) {
    try { refs = await referenciaBuffers(3); } catch (e) { refs = []; }
  }
  if (refs.length) {
    promptFinal += '\n\nUsá las imágenes adjuntas SOLO como referencia de paleta, luz y mood (son fotos reales de la marca). IGNORÁ por completo cualquier texto, logo, isotipo, la letra P o gráfica que aparezca en ellas: NO los reproduzcas.';
    for (const r of refs) {
...
const DISENOS = {
  ...
  titular:   { orden: ['kicker', 'titulo', 'acento', 'cta'],        font: 'serif', align: 'center', anchor: 'centro', titGigante: true, aFactor: 1.0 },
...
async function componerPlaca(placa, fotoBuf, formato, numero, total) {
  ...
  const vert = dis.anchor && dis.anchor !== 'auto' ? dis.anchor : (zona.vert || 'abajo');
