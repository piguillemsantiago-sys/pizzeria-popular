  ...
  properties: {
    ...
    placas: {
      ...
      items: {
        ...
        required: ['titulo', 'acento', 'bajada', 'cta', 'lugar', 'estilo', 'escenaIA', 'escenaPistas', 'banderas', 'evento'],
...
function jsonRespuesta(resp) {
  ...
  try { return JSON.parse(text); } catch (e) { return parseJsonIA(text); }
}
...
function sanearCopy(out) {
  ...
  (out.placas || []).forEach((p, i) => {
    p.titulo = normalizarEspacios(sanearTexto(p.titulo, 'el título', avisos));
    p.acento = normalizarEspacios(sanearTexto(p.acento, 'el acento', avisos));
    p.bajada = normalizarEspacios(sanearTexto(p.bajada, 'la bajada', avisos));
    p.cta = normalizarEspacios(sanearTexto(p.cta, 'el CTA', avisos));
...
const ACORTAR_SYSTEM = `Sos editor de copys de Pizzería Popular (tono argentino cálido, de "vos"). Te paso textos de placas de redes que se pasaron de su límite de palabras. Reescribí CADA uno para que quede DENTRO de su límite:
- REESCRIBÍ de verdad: reformulá, cambiá palabras, reordená. PROHIBIDO acortar borrando palabras sueltas y dejar castellano roto. Ej: "Tu pizza se va a la playa" (7) → BIEN: "Pizza para llevar a la playa" · MAL: "Tu pizza se va a playa".
...
async function acortarLargos(out) {
  ...
  try {
    const lista = ex.map((e, n) =>
      (n + 1) + '. [' + e.campo + ' — máximo ' + e.max + ' palabras, tiene ' + e.palabras + '] ' + e.texto).join('\n');
    const resp = await crearConSchema({
      model: process.env.CLAUDE_RETOQUE_MODEL || 'claude-haiku-4-5',
      max_tokens: 700,
      system: ACORTAR_SYSTEM,
      messages: [{ role: 'user', content: 'Reescribí estos textos dentro de su límite:\n' + lista }],
    }, ACORTAR_SCHEMA);
...
6. Negativos, SIEMPRE al final: "No agregues NINGÚN otro texto, palabra, número, marca de agua ni firma fuera de los textos indicados. No dibujes ningún logo, isotipo ni la letra P suelta: el logo real se agrega después. Sin caras humanas en primer plano. Sin faltas de ortografía."
...
- Si te paso INDICACIONES DE DISEÑO del usuario, tienen PRIORIDAD sobre cualquier regla de layout (nunca sobre los textos exactos).
...
- "errores" (GRAVES, obligan a regenerar): un texto esperado que falta o está MAL ESCRITO (faltas de ortografía, tildes mal, letras deformes o ilegibles, palabras cambiadas); texto INVENTADO que no está en la lista (palabras, números, precios, carteles — lo que sea legible); un logo, isotipo o letra P dibujados (el logo real se agrega después: NO debe haber ninguno); una bandera de país incorrecta si se esperaban banderas.
...
async function generarPlacaCompletaIA(placa, formato) {
  ...
  for (let intento = 1; intento <= 2; intento++) {
    ...
    const parts = refParts.length
      ? [...refParts, { text: 'Las imágenes adjuntas son placas REALES de la marca: usalas como referencia de estilo gráfico (paleta, clima, jerarquía). NO copies sus textos ni reproduzcas su logo.\n\n' + prompt }]
      : [{ text: prompt }];
  ...
  mejor.verif.errores.forEach((e) => avisos.push('⚠️ La IA no logró escribir bien: ' + e + ' — revisá la placa o probá el modo clásico.'));
  mejor.verif.avisos.forEach((a) => avisos.push(a));
  return { buf: mejor.buf, avisos };
}
...
module.exports = { generarCopy, ajustarCopy, generarPiezas, generarImagenIA, afinarPromptIA, sugerirEscenaBlog, geminiDisponible, materializarFoto, componerPlaca, fotoDePlaca, interpretarRetoque, redactarPromptPlaca, verificarPlacaIA, generarPlacaCompletaIA, terminarPlacaIA, sanearCopy, acortarLargos, FORMATOS, LOGOS, ESTILO_LIST };
