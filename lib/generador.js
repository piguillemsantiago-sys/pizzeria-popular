...
function truncarPalabras(s, max, campo, avisos) {
  ...
  // Pasarse por poco NO recorta: un corte bruto deja frases colgadas ("se va a
  // la…") y eso es peor que una palabra de más (el layout absorbe el exceso con
  // wrap/autofit). Solo se recorta un desborde grosero (más de +3), y avisando.
  if (w.length > max + 3) {
    avisos.push('Recorté ' + campo + ' de ' + w.length + ' a ' + (max + 3) + ' palabras (el máximo es ' + max + '): revisalo y acortalo mejor a mano');
    return w.slice(0, max + 3).join(' ');
  }
  if (w.length > max) {
    avisos.push('Ojo: ' + campo + ' tiene ' + w.length + ' palabras (ideal hasta ' + max + '). Lo dejé entero para no cortar la frase — acortalo a mano si querés');
  }
  return w.join(' '); // espacios simples sin extremos en todas las ramas
}
...
module.exports = { generarCopy, ajustarCopy, generarPiezas, generarImagenIA, afinarPromptIA, sugerirEscenaBlog, geminiDisponible, materializarFoto, componerPlaca, fotoDePlaca, interpretarRetoque, redactarPromptPlaca, verificarPlacaIA, generarPlacaCompletaIA, terminarPlacaIA, FORMATOS, LOGOS, ESTILO_LIST };
