...
// Regla de la comisión (dueño, 1 sep 2026): una reseña que nombra a UNA sola
// persona vale 1 punto para ella; si nombra a dos o más, vale 0,5 para cada una.
const PUNTOS_COMPARTIDA = 0.5;

// Cache en disco del descubrimiento de nombres (opcional, MENCIONES_CACHE_DIR):
// dos corridas del informe del mes tienen que dar los MISMOS nombres, y la IA
// puede variar en los de 1-2 menciones entre corridas.
function cacheDisco(local_id, desde, hasta, n) {
  const dir = process.env.MENCIONES_CACHE_DIR;
  if (!dir) return null;
  return path.join(dir, [local_id || 'todos', desde || 'x', hasta || 'x', n].join('_').replace(/[^a-z0-9_-]/gi, '_') + '.json');
}
...
// Nómina por local (dato del dueño, 1 sep 2026): gente del equipo que se cuenta
// SIEMPRE, aunque la IA no la haya descubierto en la muestra, con todas las
// formas en que los clientes la escribieron. Vale SOLO dentro de su local:
// "Mateo" es Matías en Benidorm, pero en otro local sería otra persona.
// Clave = nombre oficial (es la etiqueta que sale en el informe).
const NOMINA = {
  'benidorm': {
    'Kelly': ['keli', 'kely', 'kaily', 'keily', 'kerry', 'kally', 'kelley', 'kenny', 'kailey'],
    'Vanina': ['vani', 'banina', 'vane', 'vany', 'vanni'],
    'Valentino': ['valentin'],
    'Florencia': ['flor', 'florby'],
    'Emiliano': ['emi'],
    'Juanqui': ['juan'],
    'Matías': ['mathias', 'matia', 'mattia', 'mateo', 'mati'],
    'Luz': ['luv'],
    'Morena': [],
    'Manu': [],
  },
  'playa-san-juan': {
    'Giuliana': ['juliana'],
    // Agus/Agustina ya estaba en julio (21). En agosto la IA devolvió 'Agostina'
    // (17 reseñas) sin listar a Agus (15): puede ser otra persona o un tipeo —
    // pendiente de confirmar con el dueño; mientras tanto se cuentan aparte.
    'Agustina': ['agus'],
    'Agostina': [],
  },
};

// Nombres que además son palabras comunes ("la chica morena"): solo cuentan
// escritos con mayúscula, como ya pasa con los de 3 letras.
const SOLO_MAYUSCULA = new Set(['morena']);

// Pares [variante, canónico] que valen para un local: los apodos generales +
// los de su nómina.
const _pares = new Map();
function paresDe(local) {
  const k = local || '';
  if (_pares.has(k)) return _pares.get(k);
  const out = ALIAS.slice();
  Object.entries(NOMINA[k] || {}).forEach(([nombre, vars]) => {
    const canon = normalizar(nombre);
    vars.forEach((v) => out.push([normalizar(v), canon]));
  });
  _pares.set(k, out);
  return out;
}

// Etiqueta oficial de la nómina para un canónico, si existe.
function nombreOficial(clave, local) {
  for (const nombre of Object.keys(NOMINA[local || ''] || {})) {
    if (foldear(claveNombre(nombre, local)) === clave) return nombre;
  }
  return null;
}
...
function claveNombre(n, local) {
  const x = normalizar(n);
  for (const [corto, largo] of paresDe(local)) { if (x === corto || x === largo) return largo; }
  return x;
}
...
function formasDe(nombre, variantes, local) {
  ...
  const pares = paresDe(local);
  base.forEach((b) => {
    const k = claveNombre(b, local);
    pares.forEach(([corto, largo]) => {
      if (claveNombre(corto, local) === k || claveNombre(largo, local) === k) { conAlias.add(corto); conAlias.add(largo); }
    });
  });
  return [...conAlias];
}
...
function detector(nombre, variantes, local) {
  const formas = formasDe(nombre, variantes, local);
  const largos = formas.filter((f) => f.length > 3 && !SOLO_MAYUSCULA.has(f))
    ...
    .filter(Boolean);
  const cortos = formas.filter((f) => f.length <= 3 || SOLO_MAYUSCULA.has(f)).map((f) => f.replace(/[^a-z0-9]/g, '')).filter(Boolean);
  ...
  const reCortos = cortos.length
    ...
    : null;
  return (texto) => {
    // "Playa San Juan" es el local, no un camarero: se saca antes de buscar.
    const t = String(texto || '').replace(/\bsan\s+juan\b/gi, '');
    if (reLargos && reLargos.test(normalizar(t))) return true;
    return !!(reCortos && reCortos.test(t));
  };
}
...
function agrupar(empleados, conTexto, local) {
  const grupos = new Map();
  empleados.forEach((e) => {
    if (!e || !e.nombre) return;
    // "Cata" y "Kata" (o "Inés" e "Inez") tienen que caer en la misma persona
    // aunque la IA los haya devuelto partidos: se agrupa por cómo suenan.
    const k = foldear(claveNombre(e.nombre, local));
  ...
  const frecuencia = (forma) => {
    const test = detector(forma, [], local);
  ...
  return [...grupos.entries()].map(([k, g]) => {
    ...
    // Si la persona está en la nómina del local, manda el nombre oficial: el
    // informe tiene que decir "Kelly" aunque la gente escriba más "Keli".
    const oficial = nombreOficial(k, local);
    return { nombre: oficial || (etiqueta ? etiqueta.c : principal), nota: g.nota, variantes: candidatos };
  });
}
...
async function menciones(query = {}) {
  ...
  const maxFrases = Math.min(Math.max(parseInt(query.frases, 10) || 10, 1), 5000); // 5000 = todas (verificaciones)
  ...
  if (!equipoCrudo) {
    const disco = cacheDisco(local_id, desde, hasta, conTexto.length);
    if (disco && fs.existsSync(disco)) {
      equipoCrudo = JSON.parse(fs.readFileSync(disco, 'utf8'));
    } else {
      equipoCrudo = conTexto.length ? await descubrirEquipo(conTexto) : [];
      if (disco) { fs.mkdirSync(path.dirname(disco), { recursive: true }); fs.writeFileSync(disco, JSON.stringify(equipoCrudo, null, 1)); }
    }
    if (_cacheEquipo.size > 20) _cacheEquipo.clear();
    _cacheEquipo.set(claveEquipo, equipoCrudo);
  }
  // La nómina del local entra siempre: si a Luz la nombran una sola vez y la IA
  // no la vio en la muestra, igual tiene que sumar.
  const nomina = Object.entries(NOMINA[local_id || ''] || {})
    .map(([nombre, vars]) => ({ nombre, variantes: [nombre, ...vars], nota: '' }));
  let equipo = agrupar([...equipoCrudo, ...nomina], conTexto, local_id);

  const buscado = empleado ? claveNombre(empleado, local_id) : null;

  // Primero se cuenta para TODO el equipo: hace falta saber cuántas personas
  // nombra cada reseña (una compartida vale 0,5 para cada una). Recién después
  // se filtra si se pidió una sola persona.
  const conNombre = new Set();
  const nombresPorResena = new Map(); // id de reseña → nombres que aparecen
  let parciales = equipo.map((e) => {
    const test = detector(e.nombre, e.variantes, local_id);
    const suyas = conTexto.filter((r) => test(r.texto_original));
    suyas.forEach((r) => {
      conNombre.add(r.id);
      const l = nombresPorResena.get(r.id) || []; l.push(e.nombre); nombresPorResena.set(r.id, l);
    });
    return { e, suyas };
  }).filter((p) => p.suyas.length > 0);

  if (buscado) parciales = parciales.filter(({ e }) => claveNombre(e.nombre, local_id) === buscado ||
    (e.variantes || []).some((v) => claveNombre(v, local_id) === buscado));
  if (buscado && !parciales.length) throw err(404, 'No hay menciones de "' + empleado + '" en el período elegido');

  const empleados = parciales.map(({ e, suyas }) => {
    const compartidas = suyas.filter((r) => (nombresPorResena.get(r.id) || []).length > 1).length;
    const solas = suyas.length - compartidas;
    const estrellas = {};
    ...
    const frases = suyas
      .map((r) => ({
        ...
        otros: (nombresPorResena.get(r.id) || []).filter((n) => n !== e.nombre),
      }))
      .filter((f) => f.texto)
    ...
    return {
      ...
      solas,
      compartidas,
      puntos: solas + compartidas * PUNTOS_COMPARTIDA,
      estrellas,
  ...
  const informe = {
    ...
    totales: {
      ...
      compartidas: [...nombresPorResena.values()].filter((l) => l.length > 1).length,
      puntos: empleados.reduce((a, e) => a + e.puntos, 0),
    },
    regla_puntos: 'Reseña con una sola persona nombrada = 1 punto; con dos o más = ' + PUNTOS_COMPARTIDA + ' para cada una.',
    empleados,
...
module.exports = { menciones, textoOriginal, patronNombre, detector, NOMINA, PUNTOS_COMPARTIDA };
