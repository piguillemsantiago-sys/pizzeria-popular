...
- "elegante": para cualquier motivo con clima premium/nocturno. Campo: cursiva (frase corta manuscrita) y opcional secundario (línea chica en mayúsculas) SOLO si aporta un dato NUEVO que no está en la cursiva (fecha, hora, lugar); si repite palabras de la cursiva, va vacío.
...
async function generarPortada({ modelo, texto, foto }) {
  ...
  if (plan.modelo === 'producto') {
    ...
    // Cada modelo pintado tiene su gráfica en zonas conocidas: el logo va a la
    // zona contraria SÍ o SÍ (en impacto la banda vive abajo y el logo se le
    // montó encima el 15/7).
    const zonaLogo = plan.modelo === 'elegante' ? 'inferior' : 'superior';
    let mejor = null;
    for (let intento = 1; intento <= 2; intento++) {
      const pintada = await pintarModelo(plan, base);
      const conLogo = await ponerLogo(pintada.buf, { ancho: 380, preferencia: zonaLogo, soloZona: zonaLogo });
