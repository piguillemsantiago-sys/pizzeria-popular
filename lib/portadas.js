...
async function componerProducto(baseBuf, serif, cursiva) {
  ...
  // Presupuestos con AIRE a los costados (regla del dueño 15/7: el texto no
  // se pega a los márgenes).
  const sSize = Math.max(200, await ajustarTamano('Abril Fatface', 350, serif, 860));
  const cSize = cursiva ? Math.max(400, await ajustarTamano('Abuget', 560, cursiva, 1150)) : 0;
