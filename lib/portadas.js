...
async function elegirLogo(baseBuf, preferencia, ancho) {
  const caja = { w: ancho || 430, h: Math.round((ancho || 430) * 0.29) };
  const zonas = [
    { nombre: 'inferior', top: 1390 },
    { nombre: 'superior', top: 310 },
  ];
  if (preferencia === 'superior') zonas.reverse();
  const medidas = [];
  for (const z of zonas) {
    const stats = await sharp(baseBuf).extract({
      left: Math.round((W - caja.w) / 2), top: z.top, width: caja.w, height: caja.h,
    }).stats();
    const [r, g, b] = stats.channels;
    const lum = 0.299 * r.mean + 0.587 * g.mean + 0.114 * b.mean;
    const ruido = (r.stdev + g.stdev + b.stdev) / 3; // fondo "sucio" = logo ilegible
    medidas.push({ ...z, lum, ruido });
  }
  let zona = medidas[0];
  if (medidas[0].ruido > 55 && medidas[1].ruido < medidas[0].ruido - 15) zona = medidas[1];
  // El logo opuesto al fondo: fondo claro → oscuro, fondo oscuro → blanco.
  const variante = zona.lum > 118 ? 'oscuro' : 'blanco';
  return { top: zona.top, variante, zona: zona.nombre, lum: Math.round(zona.lum) };
}

async function ponerLogo(buf, { ancho, preferencia }) {
  const el = await elegirLogo(buf, preferencia, ancho);
...
async function componerProducto(baseBuf, serif, cursiva) {
  // La Abuget tiene colas y florituras largas: su tinta mide mucho más que su
  // cuerpo, así que su presupuesto de ancho es mayor (si no, sale enana).
  const sSize = Math.max(200, await ajustarTamano('Abril Fatface', 350, serif, 980));
  const cSize = cursiva ? Math.max(400, await ajustarTamano('Abuget', 560, cursiva, 1360)) : 0;
