...
async function elegirLogo(baseBuf, preferencia, ancho) {
  ...
  for (const c of candidatas) {
    ...
    const castigo = c.nombre === (preferencia || 'inferior') ? 0 : 18;
    medidas.push({ ...c, lum, ruido, puntaje: ruido + castigo });
  }
  ...
  return { top: zona.top, variante, zona: zona.nombre + '@' + zona.top, lum: Math.round(zona.lum) };
}
