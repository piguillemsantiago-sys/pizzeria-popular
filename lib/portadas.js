...
async function elegirLogo(baseBuf, preferencia, ancho) {
  ...
  for (const c of candidatas) {
    ...
    medidas.push({ ...c, lum: suma / n, claros: claros / n, oscuros: oscuros / n, puntaje: mezcla + castigo });
  }
  medidas.sort((a, b) => a.puntaje - b.puntaje);
  const zona = medidas[0];
  // El logo opuesto a lo que DOMINA el fondo (el promedio engaña en fondos bimodales).
  const variante = zona.claros >= zona.oscuros ? 'oscuro' : 'blanco';
