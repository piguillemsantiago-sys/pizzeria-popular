...
async function medirTexto(fontFamily, fontSize, texto) {
  ...
  const meta = await sharp(buf).trim({ threshold: 5 }).metadata();
...
// ---- Criterio de logo por MEDICIÓN, no a ojo: se mide la luminancia real de
// las dos franjas candidatas y se elige zona + variante (blanco/oscuro). ----
async function elegirLogo(baseBuf, preferencia) {
  const zonas = [
    { nombre: 'inferior', top: 1380, alto: 150 },
    { nombre: 'superior', top: 300, alto: 150 },
  ];
  if (preferencia === 'superior') zonas.reverse();
  const medidas = [];
  for (const z of zonas) {
    const stats = await sharp(baseBuf).extract({ left: 200, top: z.top, width: 680, height: z.alto }).stats();
    const [r, g, b] = stats.channels;
    const lum = 0.299 * r.mean + 0.587 * g.mean + 0.114 * b.mean;
    const ruido = (r.stdev + g.stdev + b.stdev) / 3; // franja "sucia" = mal fondo para logo
    medidas.push({ ...z, lum, ruido });
  }
  // Zona preferida salvo que esté muy sucia y la otra esté claramente mejor.
  let zona = medidas[0];
  if (medidas[0].ruido > 55 && medidas[1].ruido < medidas[0].ruido - 15) zona = medidas[1];
  const variante = zona.lum > 128 ? 'oscuro' : 'blanco';
  return { top: zona.top + 10, variante, zona: zona.nombre };
}

async function ponerLogo(buf, { ancho, preferencia }) {
  const el = await elegirLogo(buf, preferencia);
  ...
  const out = await sharp(buf).composite([
    { input: logo, left: Math.round((W - lm.width) / 2), top: el.top },
...
MODELOS DISPONIBLES:
- "producto": para platos/comida. El código compone: una palabra/frase corta en serif elegante + una palabra/frase corta en caligrafía manuscrita dorada. Campos: serif, cursiva.
...
async function componerProducto(baseBuf, serif, cursiva) {
  const sSize = await ajustarTamano('Abril Fatface', 350, serif, 960);
  const cSize = cursiva ? await ajustarTamano('Abuget', 560, cursiva, 980) : 0;
  ...
  const svg = Buffer.from(
    ...
    (cursiva ? '<text x="540" y="930" text-anchor="middle" font-family="Abuget" font-size="' + cSize + '" fill="url(#oro)" filter="url(#s2)">' + escaparXML(cursiva) + '</text>' : '') +
    '</svg>'
  );
...
async function generarPortada({ modelo, texto, foto }) {
  ...
  if (plan.modelo === 'producto') {
    ...
    const conLogo = await ponerLogo(compuesta, { ancho: 430, preferencia: 'inferior' });
    console.log('[Portadas] logo ' + conLogo.logo.variante + ' en zona ' + conLogo.logo.zona);
    final = conLogo.buf;
