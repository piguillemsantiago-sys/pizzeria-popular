...
// Carpeta raíz del banco (Fotos Sucurales). Configurable por env para poder
// recablear el banco de cada cliente sin tocar código ni redeployar. SIEMPRE debe
// apuntar a una carpeta concreta: la API de Drive lista por parent (`q: '...in parents'`),
// no soporta "todo el Drive". Si GDRIVE_BANCO_ID queda vacío, cae al ID por defecto.
const BANCO_ROOT = process.env.GDRIVE_BANCO_ID || '1VxwlWeOmexM6YvfiIbqLRWeVy3uTTC3q';
...
// ---- Fotos del banco como REFERENCIA VISUAL para Gemini ----
// Devuelve unas pocas fotos REALES de la marca (producto/ambiente/local; NUNCA
// personas, porque Gemini deforma caras; NUNCA descartables) como buffers, para
// que la imagen generada salga con la paleta/luz/mood del producto real. Es un
// filtro de tabla: NO usa IA, no suma costo de API. Solo paleta/luz/mood; el
// prompt del generador ya pide ignorar cualquier texto/logo de las referencias.
async function referenciaVisual(n) {
  n = n || 3;
  let { data: fotos } = await supabaseAdmin
    .from('ppweb_banco_imagenes')
    .select('drive_id,tipo,etiquetas')
    .in('tipo', ['producto', 'ambiente', 'local'])
    .order('usos', { ascending: true })
    .limit(120);
  fotos = (fotos || []).filter(
    (f) => !(f.etiquetas || []).some((e) => /descartable/i.test(String(e))));
  // Producto primero: la pizza real es lo más valioso como referencia de marca.
  fotos.sort((a, b) => (b.tipo === 'producto') - (a.tipo === 'producto'));

  const out = [];
  for (const f of fotos) {
    if (out.length >= n) break;
    try {
      const buf = await downloadFile(f.drive_id);
      const small = await sharp(buf).rotate()
        .resize(768, 768, { fit: 'inside', withoutEnlargement: true })
        .jpeg({ quality: 78 }).toBuffer();
      out.push(small);
    } catch (e) { /* ignora la que falle */ }
  }
  return out;
}

module.exports = { sincronizar, estado, elegirFotos, referenciaVisual };
