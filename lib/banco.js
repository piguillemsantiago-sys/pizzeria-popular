...
// Schema para structured outputs (la API garantiza JSON válido).
const VISION_SCHEMA = {
  type: 'object',
  properties: {
    descripcion: { type: 'string' },
    tipo: { type: 'string', enum: ['producto', 'personas', 'local', 'ambiente', 'otro'] },
    etiquetas: { type: 'array', items: { type: 'string' } },
  },
  required: ['descripcion', 'tipo', 'etiquetas'],
...
async function elegirFotos(instruccion, formato, placas, excluir) {
  ...
  const resp = await client.messages.create({
    model: process.env.CLAUDE_COPY_MODEL || 'claude-opus-4-8',
    max_tokens: 2000,
    ...
    output_config: { format: { type: 'json_schema', schema: ELECCION_SCHEMA } },
  });
  let text = '';
  for (const b of resp.content) if (b.type === 'text') text += b.text;
  let parsed;
  ...
  const elecciones = (parsed.elecciones || []);
