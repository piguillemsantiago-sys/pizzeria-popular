...
async function enviar(slug, opts = {}) {
  ...
  if (!g.verificacion.ok && !opts.forzar) {
    ...
    throw new Error('No pasó la verificación de datos, no se envía: '
...
async function enviarTodos() {
  ...
  // Que un informe no salga tiene que NOTARSE: el silencio se confunde con
  // que todo anduvo bien. Si alguno falló, aviso al dueño con el motivo.
  const fallados = resultados.filter((r) => r.error);
  if (fallados.length) {
    try {
      await enviarMail({
        to: COPIA,
        cc: [],
        subject: `⚠️ Informe semanal: ${fallados.length} local(es) no se enviaron`,
        html: '<div style="font-family:Arial,sans-serif;max-width:560px;margin:0 auto;">'
          + '<h2 style="color:#B3402A;">No se enviaron todos los informes</h2>'
          + '<p style="font-size:14px;color:#333;">Los demás salieron normal. Estos quedaron afuera:</p><ul>'
          + fallados.map((f) => `<li><b>${esc(f.local)}</b>: ${esc(f.error)}</li>`).join('')
          + '</ul><p style="font-size:12px;color:#999;">Aviso automático del chequeo de datos previo al envío.</p></div>',
      });
    } catch (e) {
...
module.exports = { generar, enviar, enviarTodos, semanaPasada, renderHtml, enviarMail };
