...
const DESTINOS = {
  ...
  'russafa': ['omarcrubiolo@gmail.com'],
};
...
function renderHtml(loc, d1, j) {
  ...
  // Día a día CON las franjas en la misma tabla: el total diario era la misma
  // columna en las dos, y repetirla mandaba el informe a una tercera página.
  const porDiaFranja = Object.fromEntries((((j.franjas || {}).dias) || []).map((d) => [d.dia, d]));
  const maxCelda = Object.values(porDiaFranja)
    .reduce((mx, d) => Math.max(mx, +d.man, +d.med, +d.noc), 0);
  const celda = (v) => {
    const a = maxCelda ? 0.06 + 0.5 * (Number(v) / maxCelda) : 0;
    return `<td style="background:rgba(201,145,63,${a.toFixed(3)})">${eur(v)}</td>`;
  };
  const filasDias = j.dias.map((d, i) => {
    ...
    const f = porDiaFranja[d.dia] || { man: 0, med: 0, noc: 0 };
    return `<tr><td class="dialab">${DIAS_SEMANA[i] || ''} ${Number(d.dia.slice(8))}/${Number(d.dia.slice(5, 7))}</td>
      ${celda(f.man)}${celda(f.med)}${celda(f.noc)}
      <td class="tot">${eur(d.bruto)}</td><td>${d.comensales}</td><td>${eur2(d.bruto / d.comensales)}</td>
      <td class="${clase}">${delta == null ? '—' : pct(delta)}</td></tr>`;
  }).join('\n');

  // Pie de la tabla: el total de cada franja en la semana y cómo se movió.
  const F = j.franjas || {};
  const fs_ = F.sem || {};
  const fa_ = F.ant || {};
  const totalFranjas = Number(fs_.man || 0) + Number(fs_.med || 0) + Number(fs_.noc || 0);
  const deltaFranja = (a, b) => {
    if (!(Number(b) > 0)) return '<td class="neutro">—</td>';
    const dd = 100 * (Number(a) / Number(b) - 1);
    return `<td class="${dd >= 0 ? 'sube' : 'baja'}">${pct(dd)}</td>`;
  };
  const pieFranjas = totalFranjas > 0 ? `
    <tr class="resumen"><td class="dialab">Semana</td>
      <td>${eur(fs_.man)} <span class="cuota">${pct0(100 * fs_.man / totalFranjas)}</span></td>
      <td>${eur(fs_.med)} <span class="cuota">${pct0(100 * fs_.med / totalFranjas)}</span></td>
      <td>${eur(fs_.noc)} <span class="cuota">${pct0(100 * fs_.noc / totalFranjas)}</span></td>
      <td class="tot">${eur(totalFranjas)}</td>
      <td>${(+sem.comensales).toLocaleString('es-ES')}</td><td>${eur2(ticketCom)}</td><td></td></tr>
    <tr class="delta"><td class="dialab">vs. sem. ant.</td>
      ${deltaFranja(fs_.man, fa_.man)}${deltaFranja(fs_.med, fa_.med)}${deltaFranja(fs_.noc, fa_.noc)}
      <td colspan="4"></td></tr>` : '';
...
<style>
  ...
  * { margin:0; padding:0; box-sizing:border-box; -webkit-print-color-adjust:exact; print-color-adjust:exact; }
  ...
  .fila-barra .etq .horario { font-weight:600; font-size:7.5pt; color:var(--tinta-2); }
  table.franjas td { text-align:right; font-size:8.6pt; padding:1mm 2mm; }
  table.franjas td.dialab { text-align:left; font-weight:700; }
  table.franjas th .horario { font-weight:600; font-size:6.5pt; text-transform:none; letter-spacing:0; }
  table.franjas td.tot { font-weight:700; }
  table.franjas tr.resumen td { border-top:2px solid var(--marron); font-weight:700; background:none; }
  table.franjas tr.resumen .cuota { font-weight:600; font-size:7.5pt; color:var(--tinta-2); }
  table.franjas tr.delta td { border-bottom:none; font-weight:700; background:none; }
...
<div class="pagina">
  ...
  <h2>Día a día y por turno <small>(el turno es por hora de cobro; cuanto más oscura la celda, más facturó)</small></h2>
  <table class="franjas">
    <tr><th>Día</th><th>Mañana<br><span class="horario">hasta 12</span></th><th>Mediodía<br><span class="horario">12 a 17</span></th><th>Noche<br><span class="horario">desde 18</span></th><th>Facturación</th><th>Comens.</th><th>€/com.</th><th>vs. ant.</th></tr>
    ${filasDias}
    ${pieFranjas}
  </table>
  <div class="comparado">Tickets: ${fs_.t_man || 0} de mañana · ${fs_.t_med || 0} al mediodía · ${fs_.t_noc || 0} de noche. Los tres turnos suman exactamente la facturación de la semana.</div>
...
<div class="pagina p2">
  ...
  <h2>Los que menos se vendieron <small>(solo comida de carta: los que menos rotaron)</small></h2>
  <table>
...
async function generar(slug, d1) {
  ...
  // Franjas del día (hora de COBRO, no de comanda): van aparte porque salen
  // de agora_ventas_horas, que cuadra al céntimo con el total diario.
  const { data: franjas, error: errF } = await supabaseAdmin.rpc('pp_informe_franjas',
    { p_wp: loc.wp, p_d1: d1 });
  if (errF) throw new Error('RPC franjas: ' + errF.message);
  if (data) data.franjas = franjas;
...
module.exports = { generar, enviar, enviarTodos, procesarSemana, procesarLocal, semanaPasada, renderHtml, enviarMail, DESTINOS, COPIA };
