...
async function verificar(slug, wp, d1, rpc) {
  ...
  // La semana anterior se necesita en dos lugares (cantidad de filas del PDF y
  // el "vs. semana anterior"), así que se trae una sola vez, acá arriba.
  const ant0 = await todas('agora_ventas_dias', 'business_day,total_bruto',
  ...
  // No alcanza con que los días presentes estén bien: tienen que estar TODOS.
  // Si el informe trae 6 filas, la tabla del PDF sale con un día menos y los
  // totales de arriba ya no se corresponden con lo que se ve abajo.
  const fechasInforme = (rpc.dias || []).map((r) => r.dia).sort();
  chequear(fechasInforme.length === fechas.length
    && fechasInforme.every((f, i) => f === fechas[i]), 'Días completos en el informe',
    'el informe trae ' + fechasInforme.length + ' día(s) y el TPV tiene ' + fechas.length
    + ' (faltan: ' + fechas.filter((f) => fechasInforme.indexOf(f) === -1).join(', ') + ')');
  chequear((rpc.dias_ant || []).length === ant0.length, 'Días de la semana anterior',
    'el informe trae ' + (rpc.dias_ant || []).length + ' día(s) para comparar y hay ' + ant0.length);
  ...
  chequear((rpc.top || []).length === Math.min(10, topReal.length), 'Top 10 completo',
    'el informe trae ' + (rpc.top || []).length + ' producto(s) y deberían ser '
    + Math.min(10, topReal.length));
