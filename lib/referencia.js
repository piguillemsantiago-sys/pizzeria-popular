...
// Devuelve hasta n imágenes de referencia como buffers (redimensionadas
// para no inflar el request a Gemini). Para guía visual.
async function referenciaBuffers(n) {
  const imgs = (await listarReferencias()).slice(0, n || 3);
  const out = [];
...
module.exports = { referenciaActiva, listarReferencias, referenciaBuffers, guiaDeEstilo };
