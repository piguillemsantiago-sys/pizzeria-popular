...
const { faltanParaSubir } = require('./google-stats');
const { getStats, getLatestInsight } = require('./chat-stats');
const { getWebStats } = require('./web-stats');
...
async function getOverview() {
  ...
  try { web = await getWebStats(mesKey); } catch (e) { /* sin tabla todavía */ }
  try { const i = await ig.getIgStats(mesKey); if (i && i.configurado) insta = i; } catch (e) { /* idem */ }
  ...
  return {
    ratings: ratings ? {
      ...
      locales: (ratings.locals || []).map((l) => {
        const fp = faltanParaSubir(l.rating, l.reviews);
        return {
          slug: l.slug, name: l.name, city: l.city,
          rating: l.rating, reviews: l.reviews,
          target: fp.target, faltan: fp.faltan,
          muestraReseñas: (l.topReviews || []).slice(0, 3)
            .map((r) => ({ rating: r.rating, text: String(r.text || '').slice(0, 280) }))
            .filter((r) => r.text),
        };
      }),
    } : null,
    ...
    web: web ? {
      visitas: web.visitas, visitantes: web.visitantes, whatsapp: web.whatsapp,
      reserva: web.reserva, instagram: web.instagram, formulario: web.formulario,
      previo: web.previo, paginas: web.paginas, fuentes: web.fuentes,
      dispositivos: web.dispositivos,
    } : null,
    instagram: insta ? {
      seguidores: insta.seguidores, nuevosSeguidores: insta.nuevosSeguidores,
      alcance: insta.alcance, interacciones: insta.interacciones, guardados: insta.guardados,
    } : null,
    blog,
...

{
  ...
  "reputacion": ["bullet sobre reseñas de Google: nivel, reseñas nuevas por local, cuánto falta para subir, locales a mirar"],
  "voz_cliente": { "positivo": ["hasta 3 cosas que más elogian los clientes, sacadas SOLO del texto de las reseñas de la muestra"], "negativo": ["hasta 3 quejas o cosas a mejorar, SOLO si aparecen en las reseñas de la muestra; si no hay, dejá la lista vacía []"] },
  "redes": ["bullet sobre Instagram: seguidores y su variación, alcance, interacciones, guardados"],
  "web": ["bullet sobre la web: visitas/visitantes y variación vs mes anterior, clicks a WhatsApp/reservas, páginas top, de dónde vienen"],
...
- voz_cliente: usá SOLO el texto de las reseñas de la muestra. Es una muestra chica
  y casi toda 5★, así que es normal que "negativo" quede vacío — NO inventes quejas.
...
function buildContext(ov, stats, insight, prevM, deltas) {
  ...
  if (ov.ratings) {
    ...
    ov.ratings.locales.forEach((l) => {
      const d = deltas && deltas.porLocal && deltas.porLocal[l.slug] != null
        ? ' (' + (deltas.porLocal[l.slug] >= 0 ? '+' : '') + deltas.porLocal[l.slug] + ' nuevas)' : '';
      const sub = (l.rating < 5 && l.faltan != null)
        ? ' — faltan ~' + l.faltan + ' reseñas 5★ para ' + l.target + '★ (estimado)' : '';
      L.push('- ' + l.name + ' (' + l.city + '): ' + l.rating + '★, ' + l.reviews + ' reseñas' + d + sub);
    });

    // Muestra del texto de reseñas (para "lo que dice la gente"). OJO: la API
    // pública solo trae ~5 reseñas por local, casi todas 5★ → sirve para los
    // POSITIVOS; los negativos casi no aparecen (se completan cuando entre la API).
    const conTexto = ov.ratings.locales.filter((l) => l.muestraReseñas && l.muestraReseñas.length);
    if (conTexto.length) {
      L.push('');
      L.push('MUESTRA DE LO QUE DICEN LOS CLIENTES (pocas reseñas por local, casi todas 5★):');
      conTexto.forEach((l) => {
        l.muestraReseñas.forEach((r) => {
          L.push('- [' + l.name + ' ' + r.rating + '★] ' + r.text.replace(/\s+/g, ' '));
        });
      });
    }
  } else {
  ...
  L.push('');
  if (ov.instagram) {
    const ig0 = ov.instagram;
    L.push('INSTAGRAM (mes en curso):');
    L.push('- Seguidores: ' + ig0.seguidores +
      (ig0.nuevosSeguidores != null ? ' (' + (ig0.nuevosSeguidores >= 0 ? '+' : '') + ig0.nuevosSeguidores + ' en el mes)' : '') +
      ' · Alcance: ' + ig0.alcance + ' · Interacciones: ' + ig0.interacciones + ' · Guardados: ' + ig0.guardados);
  } else {
    L.push('INSTAGRAM: sin datos.');
  }

  L.push('');
  if (ov.web) {
    const w = ov.web;
    L.push('WEB (mes en curso, tracker propio):');
    L.push('- Visitas: ' + w.visitas + ' · Visitantes: ' + w.visitantes +
      (w.previo ? ' (mes anterior: ' + w.previo.visitas + ' visitas, ' + w.previo.visitantes + ' visitantes)' : ''));
    L.push('- Clicks a WhatsApp: ' + w.whatsapp + ' · Intentos de reserva: ' + w.reserva +
      ' · Clicks a Instagram: ' + w.instagram + ' · Formularios: ' + w.formulario);
    if (w.paginas && w.paginas.length) {
      L.push('- Páginas más vistas: ' + w.paginas.slice(0, 5).map((p) => p.nombre + ' (' + p.count + ')').join(' · '));
    }
    if (w.fuentes && w.fuentes.length) {
      L.push('- De dónde vienen: ' + w.fuentes.slice(0, 5).map((f) => f.nombre + ' (' + f.count + ')').join(' · '));
    }
  } else {
    L.push('WEB: sin datos.');
  }
...
async function generarInforme() {
  ...
  const metricas = {
    ...
    webVisitas: ov.web ? ov.web.visitas : null,
    webVisitantes: ov.web ? ov.web.visitantes : null,
    igSeguidores: ov.instagram ? ov.instagram.seguidores : null,
    igAlcance: ov.instagram ? ov.instagram.alcance : null,
  };
...
async function emailInforme(row) {
  ...
  // "Lo que dice la gente": positivos + negativos sacados del texto de las reseñas.
  const vozClienteHtml = (vc) => {
    const pos = (vc && vc.positivo) || [];
    const neg = (vc && vc.negativo) || [];
    if (!pos.length && !neg.length) return '';
    const lista = (arr) => `<ul style="padding-left:20px;margin:0 0 10px;font-family:Arial,sans-serif;font-size:14px;line-height:1.6;color:#333;">`
      + arr.map((x) => `<li style="margin-bottom:4px;">${escHtml(x)}</li>`).join('') + `</ul>`;
    return `<h3 style="color:#392E2C;margin:20px 0 8px;font-family:Arial,sans-serif;">🗣️ Lo que dice la gente</h3>`
      + (pos.length ? `<p style="margin:0 0 2px;font-family:Arial,sans-serif;font-size:13px;color:#3a9d5d;"><strong>👍 Positivo</strong></p>${lista(pos)}` : '')
      + (neg.length
        ? `<p style="margin:0 0 2px;font-family:Arial,sans-serif;font-size:13px;color:#c0492f;"><strong>👎 A mejorar</strong></p>${lista(neg)}`
        : `<p style="margin:0 0 8px;font-family:Arial,sans-serif;font-size:12px;color:#999;">Sin quejas en la muestra de reseñas (se completará al activar la conexión con la API de Google).</p>`);
  };
  ...
  const html = `
    <div style="max-width:640px;margin:0 auto;font-family:Arial,sans-serif;">
      ...
      ${vozClienteHtml(inf.voz_cliente)}
      ${seccion('📷 Instagram', inf.redes)}
      ${seccion('🌐 Web', inf.web)}
