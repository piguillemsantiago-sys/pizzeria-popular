...
async function getSummary(ctx, query) {
  ...
  const { data: events, error } = await supabaseAdmin.from('menu_analytics')
    .select('event_type, menu_item_id, category_id, language, search_query, session_id, user_agent, created_at')
    .eq('restaurant_id', restaurantId)
    .gte('created_at', from.toISOString())
    .lte('created_at', to.toISOString())
    .order('created_at', { ascending: false })
    .limit(50000);
  if (error) throw httpError(500, error.message || 'Error generando analytics');
  const rows = events || [];
