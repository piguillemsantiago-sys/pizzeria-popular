...
function renderPost(post, opts) {
  ...
  // Sufijo "#split" en hero_image → hero dividido (texto izq / foto der),
  // para fotos con el sujeto al centro. Sin sufijo → hero clásico a pantalla
  // completa. El sufijo se quita de la URL real.
  const heroRaw = post.hero_image || '';
  const heroSplit = /#split$/i.test(heroRaw);
  const heroUrl = heroRaw.replace(/#split$/i, '');
  const repl = {
    HREFLANG: hreflang,
    ...
    SLUG: post.slug,
    HERO_CLASS: heroSplit ? ' article-hero--split' : '',
    HERO_IMAGE: heroUrl,
    HERO_ABS: absUrl(heroUrl),
