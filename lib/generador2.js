...
const { elegirFotos } = require('./banco');
...
{ "textos": ["cada texto EXACTO que va a aparecer en la placa, uno por elemento"], "prompt": "el prompt canon reescrito con los contenidos nuevos", "logo": "blanco" | "oscuro" }
...
async function dirigir({ tipo, modo, texto }) {
  const partes = [
    ...
    'PEDIDO DEL USUARIO:\n' + texto,
    ultimaEscena[tipo] ? 'VARIEDAD (regla del dueño): la historia anterior de este tipo usó esta escena — variá escena/paleta dentro del canon para que NO salgan iguales:\n' + ultimaEscena[tipo] : '',
  ].filter(Boolean).join('\n\n');
...
async function controlar(buf, textos, instruccionRetoque) {
...
async function generarHistoria({ tipo, modo, texto }) {
  ...
  // REAL del banco — el plato de la historia debe ser NUESTRO plato, no stock.
  let refProducto = null;
  if (tipo === 'producto') {
    try {
      const el = (await elegirFotos(
        String(texto) + '\n(Buscá una foto REAL del PRODUCTO que nombra el pedido: primeros planos o planos cortos de NUESTRA comida, el MISMO plato — si dice "sándwich de milanesa", foto de sándwich de milanesa, no otra variante. NUNCA fachadas, salones ni fotos sin comida.)',
        'historia', [{ titulo: plan.textos.join(' — ') }]
      ))[0];
      if (el && el.driveId) {
        const r = await fetch(await materializarFoto(el.driveId));
        if (r.ok) {
          ...
          refProducto = { inlineData: { mimeType: 'image/jpeg', data: small.toString('base64') } };
        }
      }
    } catch (e) {
      avisos.push(e.code === 'BANCO_VACIO'
        ? 'El banco no está indexado: la comida salió sin referencia real (tocá «Sincronizar banco»).'
        : 'Sin referencia real del producto (' + e.message + '): la comida puede salir genérica.');
    }
  }
  const NOTA_REF = 'La imagen adjunta es una foto REAL de NUESTRO producto: el plato de la historia debe verse como ESE — mismo plato, misma preparación, mismos toppings y punto de cocción, no comida genérica de stock. Podés cambiar encuadre, fondo e iluminación según el diseño, pero el producto es ese. No copies el fondo de esa foto ni ningún texto que aparezca en ella.\n\n';
  let mejor = null;
  let feedback = '';
  for (let intento = 1; intento <= 2; intento++) {
    const parts = [];
    if (refProducto) parts.push(refProducto);
    parts.push({ text: (refProducto ? NOTA_REF : '') + plan.prompt + COMUN + (feedback ? ' ERRORES DEL INTENTO ANTERIOR (corregilos sí o sí): ' + feedback : '') });
    const crudo = await llamarGemini(parts);
    const verif = await controlar(crudo, plan.textos);
