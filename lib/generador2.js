...
const COMUN = ' El QUINTO SUPERIOR del lienzo queda totalmente despejado: solo fondo, sin ningún texto ni elemento (ahí se monta después el logo real). Escribí cada texto EXACTAMENTE como está entre comillas, letra por letra, con sus tildes y eñes. No agregues NINGÚN otro texto, palabra, marca de agua, firma, logo ni letra P suelta. Sin caras humanas reconocibles en primer plano. Sin faltas de ortografía.';
...
async function generarHistoria({ tipo, modo, texto }) {
  ...
  for (let intento = 1; intento <= 2; intento++) {
    const crudo = await llamarGemini([{ text: plan.prompt + COMUN + (feedback ? ' ERRORES DEL INTENTO ANTERIOR (corregilos sí o sí): ' + feedback : '') }]);
