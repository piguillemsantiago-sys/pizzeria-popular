...
async function generarHistoria({ tipo, modo, texto }) {
  ...
  if (tipo === 'producto') {
    try {
      // La elección del banco se VALIDA con un chequeo visual antes de usarla:
      // el 14/7 eligió un sánguche de jamón para un pedido de milanesa y toda la
      // cadena downstream trabajó sobre el plato equivocado.
      const descartadas = [];
      for (let busqueda = 0; busqueda < 2 && !refProducto; busqueda++) {
        const el = (await elegirFotos(
          ...
          'historia', [{ titulo: plan.textos.join(' — ') }], descartadas
        ))[0];
        if (!el || !el.driveId) break;
        ...
        const part = { inlineData: { mimeType: 'image/jpeg', data: small.toString('base64') } };
        // ¿La foto muestra el plato pedido?
        const check = await client.messages.create({
          model: DIRECTOR_MODEL(),
          max_tokens: 120,
          system: 'Mirá la foto y respondé SOLO un JSON {"coincide": true/false}: ¿la foto muestra claramente el plato que te nombro (el plato principal de la foto ES ese)? Sé estricto: parecido no alcanza.',
          messages: [{ role: 'user', content: [
            { type: 'image', source: { type: 'base64', media_type: 'image/jpeg', data: part.inlineData.data } },
            { type: 'text', text: 'Plato pedido: ' + String(texto) },
          ] }],
        });
        let vtxt = '';
        for (const b of check.content) if (b.type === 'text') vtxt += b.text;
        let coincide = false;
        try { coincide = !!extraerJSON(vtxt).coincide; } catch (e) { coincide = true; /* sin veredicto, no bloquear */ }
        if (coincide) refProducto = part;
        else { descartadas.push(el.bancoId); console.log('[Gen2] foto del banco descartada (no es el plato):', el.bancoId); }
      }
      if (!refProducto) {
        avisos.push('No encontré en el banco una foto clara de ese plato: la comida sale sin referencia real. Si el plato está fotografiado, tocá «Sincronizar banco»; si no, sacale una foto y subila al Drive.');
      }
    } catch (e) {
