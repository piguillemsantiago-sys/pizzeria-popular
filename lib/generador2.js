...
COHERENCIA OBLIGATORIA: cada elemento de "textos" tiene que aparecer citado IDÉNTICO (letra por letra) dentro de "prompt", y "cursiva" tiene que ser exactamente uno de los "textos". Un solo texto en cursiva por placa: no metas en el prompt frases citadas que no estén en "textos".
...
// El plan del director tiene que ser coherente consigo mismo: cada texto de
// "textos" debe estar citado IGUAL dentro de "prompt", y la "cursiva" debe ser
// uno de esos textos. El 14/7 devolvió cursiva="como en casa" con
// textos[0]="El sánguche que abraza": el pintor siguió el prompt, el control
// esperaba los textos, y la placa salió marcada con errores sin culpa del pintor.
function incoherenciasDelPlan(plan) {
  const errs = [];
  for (const t of plan.textos) {
    if (!plan.prompt.includes(t)) errs.push('el texto "' + t + '" figura en "textos" pero no aparece citado idéntico dentro de "prompt"');
  }
  if (plan.cursiva && !plan.textos.some((t) => t === plan.cursiva)) {
    errs.push('la "cursiva" ("' + plan.cursiva + '") tiene que ser exactamente uno de los "textos"');
  }
  return errs;
}
...
async function dirigir({ tipo, modo, texto }) {
  ...
  const mensajes = [{ role: 'user', content: partes }];
  let plan = null;
  for (let intento = 1; intento <= 2; intento++) {
    ...
    plan = extraerJSON(out);
    if (!plan.prompt || !Array.isArray(plan.textos)) throw new Error('El director devolvió un plan incompleto.');
    const errs = incoherenciasDelPlan(plan);
    if (!errs.length) break;
    if (intento === 2) throw new Error('El plan del director salió incoherente dos veces (' + errs.join('; ') + '). Tocá «Generar historia» de nuevo.');
    console.error('[Gen2] plan incoherente, se le pide corrección al director:', errs.join(' · '));
    mensajes.push({ role: 'assistant', content: out });
    mensajes.push({ role: 'user', content: 'Tu JSON es INCOHERENTE: ' + errs.join('; ') + '. Devolvé el JSON corregido completo: cada elemento de "textos" citado idéntico dentro de "prompt", y "cursiva" igual a uno de los "textos".' });
  }
  ...
  plan.textos = plan.textos.map((t) => {
    ...
    if (fix !== t) {
      plan.prompt = plan.prompt.split(t).join(fix);
      if (plan.cursiva === t) plan.cursiva = fix;
    }
    return fix;
  });
