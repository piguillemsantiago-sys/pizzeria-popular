...
async function generarUno(localId, promos, ultimos) {
  ...
  const system = [
    ...
    '- Si una PROMO aplica a este local (leé sus condiciones con cuidado), el post es sobre esa promo → tipo "promo". Si ninguna aplica, contenido de la casa (horno de leña, masa artesanal, empanadas criollas, milanesas, el local) → tipo "evergreen".',
    '- PROHIBIDO decir "masa madre": la pizzería NO usa masa madre. Decir "masa artesanal" si hace falta hablar de la masa.',
