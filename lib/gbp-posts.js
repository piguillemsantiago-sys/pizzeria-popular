...
async function generarBorradores() {
  // El corte va contra cuándo se GENERÓ el último borrador, no contra cuándo se
  // publicó. Con franjas, el borrador nace el lunes y sale el jueves: mirando la
  // publicación, el lunes siguiente solo habían pasado 4 días y saltaba el local
  // → el "post semanal" se caía a uno cada dos semanas. Cinco días deja pasar el
  // lunes a lunes (7 días) y frena una corrida repetida de la misma semana.
  ...
  for (const localId of LOCALES) {
    try {
      ...
      const { data: rec } = await supabaseAdmin.from(TABLE).select('id')
        .eq('local_id', localId).neq('estado', 'descartado').gte('creado_en', desde).limit(1);
      if (rec && rec.length) { resultados.push({ local_id: localId, skip: 'ya se generó uno esta semana' }); continue; }
